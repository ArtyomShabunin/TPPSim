...
block control_EMA_028
  ...
  parameter Real HP_set_p = 6.7e+06 "Задание давления в контуре ВД" annotation(
  ...
  parameter Real HP_pos_start = 0.002 "Исходное УП регулирующего клапана ВД" annotation(
    Dialog(group = "Контур ВД"));
  parameter Real HP_speed_p = 1e5 / 60 "Скорость роста давления в контуре ВД" annotation(
    Dialog(group = "Контур ВД"));
  parameter Real IP_set_p = 2e+06 "Задание давления в контуре СД" annotation(
  ...
  parameter Real IP_pos_start = 0.002 "Исходное УП регулирующего клапана ВД" annotation(
    Dialog(group = "Контур СД"));
  parameter Real IP_speed_p = 0.4e5 / 60 "Скорость роста давления в контуре СД" annotation(
    Dialog(group = "Контур СД"));   
  parameter Real LP_set_p = 370000 "Задание давления в контуре НД" annotation(
  ...
  parameter Real LP_pos_start = 0.002 "Исходное УП регулирующего клапана ВД" annotation(
    Dialog(group = "Контур НД"));
  parameter Real LP_speed_p = 220 "Скорость роста давления в контуре НД" annotation(
    Dialog(group = "Контур НД"));
     
  TPPSim.Controls.pressure_control_2 HP_pressure_control(P_activation = HP_P_activation, T = 20, k = 0.000001, pos_start = HP_pos_start, set_p = HP_set_p, speed_p = HP_speed_p) annotation(
    Placement(visible = true, transformation(origin = {-30, 50}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  TPPSim.Controls.pressure_control_3 IP_pressure_control(P_activation = IP_P_activation, T = 35, k = 0.000001, pos_start = IP_pos_start, set_p = IP_set_p, speed_p = IP_speed_p) annotation(
    Placement(visible = true, transformation(origin = {30, 50}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  ...
  TPPSim.Controls.pressure_control_3 LP_pressure_control(P_activation = LP_P_activation, T = 35, k = 0.00005, pos_start = LP_pos_start, set_p = LP_set_p, speed_p = LP_speed_p) annotation(
