...
//drdh = Medium.density_derh_p(stateFlow);
    ...
    model GFHE_glob
      ...
      TPPSim.HRSG_HeatExch.GasSplitter collGas(redeclare package Medium = Medium_G, zahod = numberOfTubeSections);
...
//Газы
      for j in 1:numberOfTubeSections loop
        connect(collGas.flowOut[j], gasHE[1, j].gasIn);
        connect(gasHE[numberOfFlueSections, j].gasOut, gasOut);
        pgas_gl[numberOfFlueSections + 1, j] = gasOut.p;
      end for;
      connect(gasIn, collGas.flowIn);
...
//drdh = Medium.density_derh_p(stateFlow);
    ...
    model GasSideHE_glob "Gas Flow Heat Exchanger Side. Модель газовой стороны газо-водяного/парового теплообменника котла-утилизатора с глобальными переменными."
      ...
      Modelica.Fluid.Interfaces.FluidPort_b gasOut(redeclare package Medium = Medium) annotation(
        Placement(visible = true, transformation(extent = {{80, -20}, {120, 20}}, rotation = 0), iconTransformation(extent = {{80, -68}, {120, -28}}, rotation = 0)));
    equation
      gasOut.h_outflow = hgas_gl[section[1] + 1, section[2]];
      gasIn.p = pgas_gl[section[1], section[2]];
      gasOut.m_flow = -Dgas_gl[section[1] + 1, section[2]];
//Уравнения для потока газов
      if DynamicEnergyBalance then
...
//Граничные условия
    ...
    model GasSplitter
      ...
      outer Medium.AbsolutePressure pgas_gl "Давление газов (глобальная переменная)";
      Modelica.Fluid.Interfaces.FluidPort_b flowOut[zahod](redeclare package Medium = Medium) annotation(
        Placement(visible = true, transformation(origin = {90, -50}, extent = {{-25, -25}, {25, 25}}, rotation = 0), iconTransformation(origin = {100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Modelica.Fluid.Interfaces.FluidPort_a flowIn(redeclare package Medium = Medium) annotation(
        Placement(visible = true, transformation(origin = {-90, -50}, extent = {{-25, -25}, {25, 25}}, rotation = 0), iconTransformation(origin = {-100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
      for i in 1:zahod loop
        Dgas_gl[1, i] = flowIn.m_flow / zahod;
        hgas_gl[1, i] = inStream(flowIn.h_outflow);
      end for;
//flowIn.p = sum(pgas_gl[i, 1] for i in 1:zahod) / zahod;
      for i in 1:zahod loop
        flowOut[i].m_flow + flowIn.m_flow / zahod = 0;
        flowOut[i].h_outflow = inStream(flowIn.h_outflow);
        flowOut[i].Xi_outflow = inStream(flowIn.Xi_outflow);
