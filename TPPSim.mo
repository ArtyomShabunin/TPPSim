package TPPSim
  ...
  Параметры взяты из модели прямоточного котла для ГТЭ-110 в Thermoflex ''ВертрПрямКУсГТЭ110_OD''
    ...
    model twoPDrumStartUp
      ...
      TPPSim.Pumps.simplePump FWPump(redeclare package Medium = Medium_F, setD_flow = 5) annotation(
    ...
    equation
      connect(GT.flowOut, SH.gasIn) annotation(
        Line(points = {{62, 12}, {44, 12}, {44, 12}, {44, 12}}, color = {0, 127, 255}));
      connect(Drum.HPFW, FWPump.port_a) annotation(
        Line(points = {{-10, 42}, {-10, 42}, {-10, -18}, {-6, -18}, {-6, -16}}, color = {0, 127, 255}));
      connect(FWPump.port_b, flowSink.ports[2]) annotation(
        Line(points = {{4, -16}, {54, -16}, {54, 54}, {60, 54}, {60, 56}}, color = {0, 127, 255}));
...
The sensor is ideal, i.e., it does not influence the fluid.
  ...
  package Pumps
    model simplePump
      ...
      replaceable package Medium = Modelica.Media.Water.StandardWater constrainedby Modelica.Media.Interfaces.PartialMedium;
      parameter Modelica.SIunits.MassFlowRate setD_flow = 1 "Производительность насоса";
      Modelica.Fluid.Interfaces.FluidPort_a port_a(redeclare package Medium = Medium) annotation(
        Placement(visible = true, transformation(origin = {100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Modelica.Fluid.Interfaces.FluidPort_b port_b(redeclare package Medium = Medium) annotation(
        Placement(visible = true, transformation(origin = {-100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
      port_b.m_flow = -setD_flow;
      port_a.m_flow = -port_b.m_flow;
      port_b.h_outflow = inStream(port_a.h_outflow);
//port_b.h_outflow = 168e3;
