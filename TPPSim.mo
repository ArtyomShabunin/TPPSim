   ...
   by Artyom Shabunin:<br></li>
    ...
    equation
      if DynamicEnergyBalance == true then
        ...
        0 = 0.5 * alfa_flow * deltaSFlow * (t_m - stateFlow.T) - D_flow_v * (h_v - h_gl[section[1], section[2]]);
        0 = 0.5 * alfa_flow * deltaSFlow * (t_m - stateFlow.T) - D_flow_v * (h_gl[section[1], section[2] + 1] - h_v);
      end if;
//Уравнение теплового баланса металла
      if DynamicTm == true then
        deltaMMetal * C_m * der(t_m) = Q_flow - alfa_flow * deltaSFlow * (t_m - stateFlow.T) "Уравнение баланса тепла металла (формула 3-2в диссертации Рубашкина)";
...
//Уравнения для расчета процессов массообмена
    ...
    equation
      if DynamicEnergyBalance then
        deltaVGas * Medium.density(state) * Medium.heatCapacity_cp(state) * der(T_out) = Dgas_gl[section[1], section[2]] * (hgas_gl[section[1], section[2]] - hgas_gl[section[1] + 1, section[2]]) + heat.Q_flow;
      else
        0 = Dgas_gl[section[1], section[2]] * (hgas_gl[section[1], section[2]] - hgas_gl[section[1] + 1, section[2]]) + heat.Q_flow;
      end if;
      heat.Q_flow = -alfa_gas * H_fin * (T_out - heat.T);
//Уравнения состояния
      state = Medium.setState_pTX(pgas_gl[section[1] + 1, section[2]], T_out, actualStream(gasIn.Xi_outflow));
      hgas_gl[section[1] + 1, section[2]] = Medium.specificEnthalpy(state);
      drdp = Medium.density_derp_T(state);
      drdT = Medium.density_derT_p(state);
      if DynamicMassBalance then
        (-Dgas_gl[section[1] + 1, section[2]]) + Dgas_gl[section[1], section[2]] - deltaVGas * (drdT * der(T_out) + drdp * der(pgas_gl[section[1], section[2]])) = 0 "Уравнение сплошности";
      else
