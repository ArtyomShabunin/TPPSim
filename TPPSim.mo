  ...
  Параметры взяты из модели прямоточного котла для ГТЭ-110 в Thermoflex ''ВертрПрямКУсГТЭ110_OD''
    ...
    equation
      connect(flowSink.ports[1], HP_EVO.flowOut) annotation(
        Line(points = {{160, 50}, {74, 50}, {74, 20}, {74, 20}}, color = {0, 127, 255}, thickness = 0.5));
      connect(flowSource.ports[1], HP_EVO.flowIn) annotation(
        Line(points = {{-74, 50}, {66, 50}, {66, 20}, {66, 20}}, color = {0, 127, 255}, thickness = 0.5));
      connect(HP_EVO.gasOut, gasSink.ports[1]) annotation(
        Line(points = {{66, 10}, {-78, 10}, {-78, 10}, {-80, 10}}, color = {0, 127, 255}));
...
//drdh = Medium.density_derh_p(stateFlow);
    ...
    protected
      ...
      inner parameter Modelica.SIunits.Volume deltaVGas = deltaLpipe * (s1 * s2 - Modelica.Constants.pi * (Din + 2 * delta) ^ 2 / 4) * z1 "Объем одного участка газового тракта";
      inner parameter Modelica.SIunits.Area f_gas = (1 - (Din + 2 * delta) / s1 * (1 + 2 * hfin * delta_fin / sfin / (Din + 2 * delta))) * deltaLpipe * s2 * z1 "Площадь для прохода газов";
      parameter Integer[numberOfFlueSections, numberOfTubeSections, 2] section_set = coorSecGen(numberOfFlueSections, numberOfTubeSections);
      //Характеристики оребрения
      inner parameter Real H_fin = (omega * deltaLpipe * (1 - delta_fin / sfin) + (2 * Modelica.Constants.pi * (Dfin ^ 2 - (Din + 2 * delta) ^ 2) / 4 + Modelica.Constants.pi * Dfin * delta_fin) * (deltaLpipe / sfin)) * z1 * zahod "Площадь оребренной поверхности";
      //Переменные
