  ...
  package HRSG_HeatExch
    package BaseClases
      ...
      partial model BaseFlowSideHE
        ...
        constant Modelica.SIunits.SpecificEnthalpy hzero = 1e-3 "Small value for deltah";
        //Характеристики металла
        final outer parameter Modelica.SIunits.SpecificHeatCapacity C_m "Удельная теплоемкость металла" annotation(
          Dialog(group = "Металл"));
        final outer parameter Modelica.SIunits.ThermalConductivity lambda_m "Теплопроводность метала" annotation(
          Dialog(group = "Металл"));
        //Конструктивные характеристики
