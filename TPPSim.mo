package TPPSim
  ...
  Параметры взяты из модели прямоточного котла для ГТЭ-110 в Thermoflex ''ВертрПрямКУсГТЭ110_OD''
    ...
    model twoPDrumStartUp
      ...
      TPPSim.flowLimiter flowLimiter2(redeclare package Medium = Medium_F, m_flow_small = 0.01) annotation(
        Placement(visible = true, transformation(origin = {153, 45}, extent = {{-3, 3}, {3, -3}}, rotation = 0)));
    ...
    equation
      ...
      connect(flowLimiter2.waterOut, CV2.port_a) annotation(
        Line(points = {{152, 48}, {152, 48}, {152, 56}, {152, 56}}, color = {0, 127, 255}));
      connect(HP_SH.flowOut, flowLimiter2.waterIn) annotation(
        Line(points = {{152, 24}, {154, 24}, {154, 42}, {154, 42}}, color = {0, 127, 255}));
      connect(flowLimiter1.waterOut, CV1.port_a) annotation(
        Line(points = {{42, 48}, {42, 48}, {42, 56}, {42, 56}}, color = {0, 127, 255}));
      connect(SH.flowOut, flowLimiter1.waterIn) annotation(
        Line(points = {{42, 24}, {42, 24}, {42, 42}, {44, 42}}, color = {0, 127, 255}));
