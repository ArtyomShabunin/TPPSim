...
model ParallelGFHE_simple "Модель двух параллельных теплообменников котла-утилизатора."
  ...
  final parameter Real x_gasflow_1 = (1 - (Din_1 + 2 * delta_1) / s1_1 * (1 + 2 * hfin_1 * delta_fin_1 / sfin_1 / (Din_1 + 2 * delta_1))) / (1 - (Din_1 + 2 * delta_1) / s1_1 * (1 + 2 * hfin_1 * delta_fin_1 / sfin_1 / (Din_1 + 2 * delta_1)) + 1 - (Din_2 + 2 * delta_2) / s1_2 * (1 + 2 * hfin_2 * delta_fin_2 / sfin_2 / (Din_2 + 2 * delta_2))) "Относительный расход газов через первый теплообменник";
  //Интерфейс
  ...
  Modelica.Fluid.Interfaces.FluidPort_b flowOut_2(redeclare package Medium = Medium_F) annotation(
    Placement(visible = true, transformation(origin = {50, -80}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {40, -100}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  TPPSim.HRSG_HeatExch.GFHE_simple GFHE_1(redeclare TPPSim.HRSG_HeatExch.FlowSide2phHE flowHE(redeclare TPPSim.thermal.alfaForSHandECO alpha), redeclare package Medium_G = Medium_G, redeclare package Medium_F = Medium_F, Din = Din_1, HRSG_type_set = HRSG_type_set_1, Lpipe = Lpipe_1, delta = delta_1, delta_fin = delta_fin_1, flowEnergyDynamics = flowEnergyDynamics_1, flowMassDynamics = flowMassDynamics_1, flowMomentumDynamics = flowMomentumDynamics_1, gasEnergyDynamics = gasEnergyDynamics_1, gasMassDynamics = gasMassDynamics_1, hfin = hfin_1, k_gamma_gas = k_gamma_gas_1, k_volume = k_volume_1, k_volume_gas = k_volume_gas_2, k_weight_metal = k_weight_metal_1, ke = ke_1, metalDynamics = metalDynamics_1, numberOfVolumes = numberOfVolumes_1, s1 = s1_1, s2 = s2_1, sfin = sfin_1, z1 = z1_1, z2 = z2_1, zahod = zahod_1) annotation(
    Placement(visible = true, transformation(origin = {-7.10543e-15, 46}, extent = {{-34, -34}, {34, 34}}, rotation = 0)));
  TPPSim.HRSG_HeatExch.GFHE_simple GFHE_2(redeclare TPPSim.HRSG_HeatExch.FlowSide2phHE flowHE(redeclare TPPSim.thermal.alfaForSHandECO alpha), redeclare package Medium_G = Medium_G, redeclare package Medium_F = Medium_F, Din = Din_2, HRSG_type_set = HRSG_type_set_2, Lpipe = Lpipe_2, delta = delta_2, delta_fin = delta_fin_2, flowEnergyDynamics = flowEnergyDynamics_2, flowMassDynamics = flowMassDynamics_2, flowMomentumDynamics = flowMomentumDynamics_2, gasEnergyDynamics = gasEnergyDynamics_2, gasMassDynamics = gasMassDynamics_2, hfin = hfin_2, k_gamma_gas = k_gamma_gas_2, k_volume = k_volume_2, k_volume_gas = k_volume_gas_2, k_weight_metal = k_weight_metal_2, ke = ke_2, metalDynamics = metalDynamics_2, numberOfVolumes = numberOfVolumes_2, s1 = s1_2, s2 = s2_2, sfin = sfin_2, z1 = z1_2, z2 = z2_2, zahod = zahod_2) annotation(
