...
model FlowSideSH_glob
  extends TPPSim.HRSG_HeatExch.BaseClases.BaseFlowSideHE_glob(redeclare replaceable package Medium = Modelica.Media.Water.StandardWater constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model");
...
equation
  if DynamicEnergyBalance then
    if noEvent(SH_cold) then
      deltaVFlow * stateFlow.d * der(stateFlow.h) = -D_flow_v * (h_gl[section[1], section[2] + 1] - h_gl[section[1], section[2]]);
    else
      deltaVFlow * stateFlow.d * der(stateFlow.h) = alfa_flow * deltaSFlow * (t_m - stateFlow.T) - D_flow_v * (h_gl[section[1], section[2] + 1] - h_gl[section[1], section[2]]);
    end if;
  else
    if noEvent(SH_cold) then
      h_gl[section[1], section[2]] = h_gl[section[1], section[2] + 1];
    else
      D_flow_v * (h_gl[section[1], section[2] + 1] - h_gl[section[1], section[2]]) = alfa_flow * deltaSFlow * (t_m - stateFlow.T);
    end if;
  end if;
