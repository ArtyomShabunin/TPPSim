package MyHRSG
  ...
  function lambda_tr "Функция для расчета коэффициента трения (лямбда)"
    ...
    protected
      ...
      //Integer k;
      Real v[66];
      Real c[66] = {0.01796402679000598, 0.02108958979778396, -0.04583701469697094, -0.01552626530766092, 0.03469040392357215, 0.008298586720201631, 0.007815172283880148, -0.02474704527602503, 0.006901442573316034, 0.009461869493831340, -0.002873866111472054, 0.01148064940037301, -0.003433293929292430, -0.02502857345079169, 0.02437164019013025, 0.0008320866302348799, -0.003778224989136373, 0.0005274177160390015, 0.01711984322359736, -0.03734584080611467, 0.01652605875273037, -0.0002242153523536768, 0.0009800840130089757, 0.0003641025065353705, -0.008767961590885893, 0.02326847113458818, -0.03061540578919750, 0.01131986978840101, 4.002270861490635E-05, -0.0002563256953249891, -0.0002889308784323206, 0.003326709465320984, -0.01036296939772042, 0.01782711934254719, -0.01638397958566612, 0.003225785705301679, -9.680790961112502E-06, 7.503325008422032E-05, 0.0001282879491034502, -0.001001907728724629, 0.003310577537412439, -0.006873012758034756, 0.008497131082831614, -0.006148028724046763, 0.001317599642793501, 2.822243354405768E-05, -1.882823794392064E-05, -5.364619785339027E-05, 0.0003018865587555519, -0.0007993159346449960, 0.001679358947221379, -0.002730025089066175, 0.002886841595805149, -0.001385592809039147, -5.700084038620243E-05, -1.082268186830316E-05, -1.400883943888743E-06, 2.551440803111890E-05, -7.656197390652729E-05, 0.0001387982545447798, -0.0002286531571855085, 0.0004248466938074243, -0.0005835534777787180, 0.0002761513300102546, -1.540798609711780E-05, 4.743673713185089E-05};
    algorithm
      if (log(ke / dn) - 7.110487833036219) / 2.505317647048128 < (-1) then
        x := -1;
      elseif (log(ke / dn) - 7.110487833036219) / 2.505317647048128 > 1 then
        x := 1;
      else
        x := (log(ke / dn) - 7.110487833036219) / 2.505317647048128;
      end if;
      if (log(Re) - 11.44416055169631) / 4.759548824028382 < (-1) then
        y := -1;
      elseif (log(Re) - 11.44416055169631) / 4.759548824028382 > 1 then
        y := 1;
      else
        y := (log(Re) - 11.44416055169631) / 4.759548824028382;
      end if;
      tx[1] := 1;
      ty[1] := 1;
      tx[2] := x;
      ...
      lambda := 0;
      for i in 1:66 loop
        lambda := lambda + c[i] * v[i];
      end for;
      /*for i in 1:10 loop
                                    tx[i] := cos(i * acos(x));
                                    ty[i] := cos(i * acos(y));
                                  end for;
                                  lambda := c[1] + c[2] * tx[1] + c[3] * ty[1] + c[4] * tx[2] + c[5] * tx[1] * ty[1] + c[6] * ty[2] + c[7] * tx[3] + c[8] * tx[2] * ty[1] + c[9] * tx[1] * ty[2] + c[10] * ty[3] + c[11] * tx[4] + c[12] * tx[3] * ty[1] + c[13] * tx[2] * ty[2] + c[14] * tx[1] * ty[3] + c[15] * ty[4] + c[16] * tx[5] + c[17] * tx[4] * ty[1] + c[18] * tx[3] * ty[2] + c[19] * tx[2] * ty[3] + c[20] * tx[1] * ty[4] + c[21] * ty[5] + c[22] * tx[6] + c[23] * tx[5] * ty[1] + c[24] * tx[4] * ty[2] + c[25] * tx[3] * ty[3] + c[26] * tx[2] * ty[4] + c[27] * tx[1] + ty[5] + c[28] * ty[6] + c[29] * tx[7] + c[30] * tx[6] * ty[1] + c[31] * tx[5] * ty[2] + c[32] * tx[4] * ty[3] + c[33] * tx[3] * ty[4] + c[34] * tx[2] * ty[5] + c[35] * tx[1] * ty[6] + c[36] * ty[7] + c[37] * tx[8] + c[38] * tx[7] * ty[1] + c[39] * tx[6] * ty[2] + c[40] * tx[5] * ty[3] + c[41] * tx[4] * ty[4] + c[42] * tx[3] * ty[5] + c[43] * tx[2] * ty[6] + c[44] * tx[1] * ty[7] + c[45] * ty[8] + c[46] * tx[9] + c[47] * tx[8] * ty[1] + c[48] * tx[7] * ty[2] + c[49] * tx[6] * ty[3] + c[50] * tx[5] * ty[4] + c[51] * tx[4] * ty[5] + c[52] * tx[3] * ty[6] + c[53] * tx[2] * ty[7] + c[54] * tx[1] * ty[8] + c[55] * ty[9] + c[56] * tx[10] + c[57] * tx[9] * ty[1] + c[58] * tx[8] * ty[2] + c[59] * tx[7] * ty[3] + c[60] * tx[6] * ty[4] + c[61] * tx[5] * ty[5] + c[62] * tx[4] * ty[6] + c[63] * tx[3] * ty[7] + c[64] * tx[2] * ty[8] + c[65] * tx[1] * ty[9] + c[66] * ty[10];*/
    end lambda_polinom;
  protected
    Real Re_I "Нижняя граница переходной области (по числу Рейнольдса)";
    Real Re_II "Верхняя граница переходной области (по числу Рейнольдса)";
