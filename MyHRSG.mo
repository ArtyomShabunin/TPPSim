package MyHRSG
  ...
  model testOnlyGasHE
    ...
    MyHRSG.onlyGasHE onlygashe1(redeclare package Medium_G = Medium_G, numberOfTubeSections = numberOfTubeSections, numberOfFlueSections = numberOfFlueSections, z2 = z2) annotation(Placement(visible = true, transformation(origin = {0, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  model TestOnlyHEBoil
    ...
    parameter Modelica.SIunits.MassFlowRate wflow = 70 "Номинальный массовый расход воды/пар";
    ...
    parameter SI.Temperature Toutflow = 60 + 273.15 "Начальная выходная температура потока воды/пар";
    ...
    //**
    parameter Integer numberOfTubeSections = 1 "Число участков разбиения трубы" annotation(Dialog(group = "Конструктивные характеристики"));
    parameter Integer numberPMCalcSections = 1 "Число участков разбиения трубы входящих в один участок расчета процессов массообмена";
    parameter Integer numberOfFlueSections = z2 "Число участков разбиения газохода" annotation(Dialog(group = "Конструктивные характеристики"));
    parameter Integer z2 = 1 "Число труб по ходу газов в данной поверхности нагрева";
    parameter Integer zahod = 1 "заходность труб теплообменника";
    //**/
    parameter Modelica.SIunits.Time Tstab_set = 1000;
    //***
    //**
    ThermoPower.Water.SourceMassFlow flowsource(h = hinflow, p0 = pflow, use_in_h = true, use_in_w0 = false, w0 = wflow) annotation(Placement(visible = true, transformation(origin = {-88, 60}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    Modelica.Blocks.Sources.Step flowenthalpstep(height = 3000, offset = hinflow, startTime = 700) annotation(Placement(visible = true, transformation(origin = {-90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    inner ThermoPower.System system(initOpt = ThermoPower.Choices.Init.Options.steadyState) annotation(Placement(visible = true, transformation(origin = {90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    ThermoPower.Water.ValveLin flowvalve(redeclare package Medium = Medium_F, Kv = wflow / 0.5 / pflow) annotation(Placement(visible = true, transformation(origin = {34, -90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    Modelica.Blocks.Sources.Step flowvalvestep(height = 0, offset = 1, startTime = 700) annotation(Placement(visible = true, transformation(origin = {4, -54}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    MyHRSG.onlyFlowHEBoil onlyFlowHE1(redeclare package Medium_F = Medium_F, setD_flow = wflow, setp_flow = pflow, setT_inFlow = Tinflow, setT_outFlow = Toutflow, numberOfTubeSections = numberOfTubeSections, z2 = z2, zahod = zahod, numberPMCalcSections = numberPMCalcSections, Tstab = Tstab_set) annotation(Placement(visible = true, transformation(origin = {0, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    MyHRSG.pipe pipe(setD = wflow, setp = pflow, setT = Tinflow, numberOfTubeSections = 2, Din = 0.2) annotation(Placement(visible = true, transformation(origin = {0, 40}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
    Modelica.Thermal.HeatTransfer.Sources.FixedHeatFlow heatSource[numberOfFlueSections, numberOfTubeSections](Q_flow = 2e8 / numberOfTubeSections / numberOfFlueSections) annotation(Placement(visible = true, transformation(origin = {38, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  equation
    connect(pipe.waterOut, onlyFlowHE1.waterIn) annotation(Line(points = {{0, 27}, {0, 27}, {0, 10}, {0, 10}}, color = {0, 127, 255}));
    connect(sensflow1.outlet, pipe.waterIn) annotation(Line(points = {{-40, 60}, {0, 60}, {0, 54}, {0, 54}}, color = {0, 0, 255}));
    for j in 1:numberOfTubeSections loop
      for i in 1:numberOfFlueSections loop
...
//Граничные условия
  ...
  model onlyFlowHEBoil
    ...
    //**
    //Параметры
    parameter MyHRSG.Choices.HRSG_type HRSG_type = MyHRSG.Choices.HRSG_type.horizontalBottom "Тип КУ";
    ...
    //**
    //Поток вода/пар
    //parameter Medium_F.SpecificEnthalpy h_startFlow_n[numberOfFlueSections, numberOfTubeSections + 1] = MyHRSG.razbivka_n(seth_in, seth_out,zahod, numberOfFlueSections, numberOfTubeSections + 1) "Начальный вектор энальпии потока газов" annotation(Dialog(tab = "Инициализация"));
    ...
    //parameter Medium_F.AbsolutePressure p_startFlow_n[numberOfFlueSections, numberOfTubeSectionsForMT + 1] = MyHRSG.razbivka_n(setp_flow_in, setp_flow_out,zahod, numberOfFlueSections, numberOfTubeSectionsForMT + 1) "Начальный вектор давлений потока вода/пар" annotation(Dialog(tab = "Инициализация"));
    //parameter Medium_F.AbsolutePressure p_startFlow_v[numberOfFlueSections, numberOfTubeSectionsForMT] = MyHRSG.razbivka_v(setp_flow_in, setp_flow_out,zahod, numberOfFlueSections, numberOfTubeSectionsForMT) "Начальный вектор давлений потока вода/пар" annotation(Dialog(tab = "Инициализация"));
...
//Описание гибов
      ...
      if HRSG_type == Choices.HRSG_type.horizontalBottom or HRSG_type == MyHRSG.Choices.HRSG_type.horizontalTop then
...
//Граничные условия для высотной отметки входного коллектора
  ...
  model GF_HE
    ...
    //***конструктивные характеристики
    //**
    parameter MyHRSG.Choices.HRSG_type HRSG_type_set = Choices.HRSG_type.horizontalBottom "Выбор типа КУ (горизонтальный/вертикальный)";
  ...
  model testOneGFHE
    ...
    parameter SI.MassFlowRate wgas = 509 "Номинальный (и начальный) массовый расход газов";
    parameter SI.Pressure pgas = 3e3 "Начальное давление газов";
    parameter SI.Temperature Tingas = Tinflow "Начальная входная температура газов";
    parameter SI.Temperature Toutgas = Toutflow "Начальная выходная температура газов";
    parameter Real k_gamma_gas = 1 "Поправка к коэффициенту теплоотдачи со стороны газов";
    parameter Real Set_X[6] = Medium_G.reference_X "Состав дымовых газов";
    //**
    //***Исходные данные для водяной стороны
    //**
    replaceable package Medium_F = Modelica.Media.Water.WaterIF97_ph constrainedby Modelica.Media.Interfaces.PartialMedium;
    parameter Modelica.SIunits.MassFlowRate wflow = 0 "Номинальный массовый расход воды/пар";
    parameter Modelica.SIunits.Pressure pflow = 10e5 "Начальное давление потока вода/пар";
    parameter Medium_F.SaturationProperties sat_start = Medium_F.setSat_p(pflow);
    parameter SI.Temperature Tinflow = Medium_F.saturationTemperature_sat(sat_start) "Начальная входная температура потока воды/пар";
    parameter SI.Temperature Toutflow = Tinflow "Начальная выходная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow = Medium_F.dewEnthalpy(sat_start) "Начальная энтальпия входного потока вода/пар";
    //**
    //***Исходные данные по разбиению
    //**
    parameter Integer numberOfTubeSections = 3 "Число участков разбиения трубы" annotation(Dialog(group = "Конструктивные характеристики"));
    ...
    parameter Integer zahod = 1 "заходность труб теплообменника";
    parameter Integer z1 = 126 "Число труб по ширине газохода";
    parameter Integer z2 = 1 "Число труб по ходу газов в теплообменнике";
    parameter Modelica.SIunits.Length Lpipe = 20.85 "Длина теплообменной трубки";
    ///Оребрение
    parameter Modelica.SIunits.Length delta_fin = 0.0008 "Средняя толщина ребра, м";
    parameter Modelica.SIunits.Length hfin = 0.017 "Высота ребра, м";
    ...
    //***
    //**
    inner ThermoPower.System system(allowFlowReversal = false) annotation(Placement(visible = true, transformation(origin = {90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    Modelica.Blocks.Sources.Step gastempstep(height = 100, offset = Tingas, startTime = 100) annotation(Placement(visible = true, transformation(origin = {-90, -28}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    //ThermoPower.Water.SourceMassFlow flowsource(h = hinflow, p0 = pflow, use_in_h = true, use_in_w0 = true, w0 = wflow) annotation(Placement(visible = true, transformation(origin = {-88, 60}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ThermoPower.Water.SensT sensflow1(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {-46, 64}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Step flowenthalpstep(height = 0, offset = Tinflow, startTime = 500) annotation(Placement(visible = true, transformation(origin = {-90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ThermoPower.Water.SinkPressure sinkflow(p0 = pflow) annotation(Placement(visible = true, transformation(origin = {90, -90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ThermoPower.Water.SensT sensflow2(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {60, -86}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    GF_HE HE(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas, Toutgas = Toutgas, k_gamma_gas = k_gamma_gas, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow, Tinflow = Tinflow, Toutflow = Toutflow, numberOfTubeSections = numberOfTubeSections, numberPMCalcSections = numberPMCalcSections, numberOfFlueSections = numberOfFlueSections, Din = Din, delta = delta, s1 = s1, s2 = s2, zahod = zahod, z1 = z1, z2 = z2, Lpipe = Lpipe, delta_fin = delta_fin, hfin = hfin, sfin = sfin, seth = hflow) annotation(Placement(visible = true, transformation(origin = {0, 0}, extent = {{-30, -30}, {30, 30}}, rotation = 0)));
    Modelica.Blocks.Sources.Ramp ramp1(duration = 40, height = 50, offset = wflow, startTime = 10) annotation(Placement(visible = true, transformation(origin = {-76, 32}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T boundary(redeclare package Medium = Medium_F, nPorts = 1, use_T_in = true, use_m_flow_in = true) annotation(Placement(visible = true, transformation(origin = {-76, 60}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(sensflow1.inlet, boundary.ports[1]) annotation(Line(points = {{-52, 60}, {-66, 60}, {-66, 60}, {-66, 60}}, color = {0, 0, 255}));
    connect(flowenthalpstep.y, boundary.T_in) annotation(Line(points = {{-78, 90}, {-70, 90}, {-70, 72}, {-90, 72}, {-90, 64}, {-88, 64}}, color = {0, 0, 127}));
    connect(ramp1.y, boundary.m_flow_in) annotation(Line(points = {{-86, 32}, {-96, 32}, {-96, 68}, {-86, 68}, {-86, 68}}, color = {0, 0, 127}));
    connect(HE.flowOut, sensflow2.inlet) annotation(Line(points = {{34, -14}, {44, -14}, {44, -90}, {54, -90}, {54, -90}}, color = {0, 127, 255}));
//connect(ramp1.y, flowsource.in_w0) annotation(Line(points = {{-88, 32}, {-98, 32}, {-98, 74}, {-92, 74}, {-92, 66}, {-92, 66}}, color = {0, 0, 127}));
    connect(sensflow1.outlet, HE.flowIn) annotation(Line(points = {{-40, 60}, {-42, 60}, {-42, -14}, {-34, -14}, {-34, -14}}, color = {0, 0, 255}));
    connect(HE.gasOut, gasvalve.inlet) annotation(Line(points = {{33, 15}, {46, 15}, {46, 40}, {28, 40}, {28, 58}, {28, 58}}, color = {0, 127, 255}));
    connect(sensgas1.outlet, HE.gasIn) annotation(Line(points = {{-52, -60}, {-46, -60}, {-46, 12}, {-34, 12}, {-34, 12}}, color = {159, 159, 223}));
...
//connect(flowenthalpstep.y, flowsource.in_h) annotation(Line(points = {{-79, 90}, {-72, 90}, {-72, 74}, {-84, 74}, {-84, 66}, {-84, 66}}, color = {0, 0, 127}));
  ...
  model testThreeGFHE
    ...
    parameter Integer zahod = 1 "заходность труб теплообменника";
    parameter Integer z1 = 126 "Число труб по ширине газохода";
    parameter Integer z2 = 10 "Число труб по ходу газов в теплообменнике";
    parameter Modelica.SIunits.Length Lpipe = 20.85 "Длина теплообменной трубки";
    ///Оребрение
    parameter Modelica.SIunits.Length delta_fin = 0.0008 "Средняя толщина ребра, м";
    parameter Modelica.SIunits.Length hfin = 0.017 "Высота ребра, м";
    ...
    ThermoPower.Gas.SensT sensgas2(redeclare package Medium = Medium_G) annotation(Placement(visible = true, transformation(origin = {66, 62}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ThermoPower.Gas.SourceMassFlow gassource(redeclare package Medium = Medium_G, p0 = pgas, T = Tingas, w0 = wgas, use_in_T = true, use_in_X = true) annotation(Placement(visible = true, transformation(origin = {-90, -60}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Step gastempstep(height = 100, offset = Tingas, startTime = 400) annotation(Placement(visible = true, transformation(origin = {-90, -28}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ThermoPower.Gas.SensT sensgas1(redeclare package Medium = Medium_G) annotation(Placement(visible = true, transformation(origin = {-58, -56}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Constant[6] setX(k = Set_X) annotation(Placement(visible = true, transformation(origin = {-90, -88}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    ThermoPower.Water.SensT sensflow1(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {-46, 64}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Step flowenthalpstep(height = 1000, offset = hinflow, startTime = 500) annotation(Placement(visible = true, transformation(origin = {-90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ThermoPower.Water.SinkPressure sinkflow(p0 = pflow / 2, redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {90, -90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ThermoPower.Water.SensT sensflow2(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {60, -86}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    Modelica.Blocks.Sources.Step gasvalvestep(height = 0, offset = 1, startTime = 40) annotation(Placement(visible = true, transformation(origin = {18, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    GF_HE HE1(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas, Toutgas = Toutgas, k_gamma_gas = k_gamma_gas, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow, Tinflow = Tinflow, Toutflow = Toutflow, numberOfTubeSections = numberOfTubeSections, numberPMCalcSections = numberPMCalcSections, numberOfFlueSections = numberOfFlueSections, Din = Din, delta = delta, s1 = s1, s2 = s2, zahod = zahod, z1 = z1, z2 = z2, Lpipe = Lpipe, delta_fin = delta_fin, hfin = hfin, sfin = sfin) annotation(Placement(visible = true, transformation(origin = {-50, 0}, extent = {{-15, -15}, {15, 15}}, rotation = 0)));
    GF_HE HE2(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas, Toutgas = Toutgas, k_gamma_gas = k_gamma_gas, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow, Tinflow = Tinflow, Toutflow = Toutflow, numberOfTubeSections = numberOfTubeSections, numberPMCalcSections = numberPMCalcSections, numberOfFlueSections = numberOfFlueSections, Din = Din, delta = delta, s1 = s1, s2 = s2, zahod = zahod, z1 = z1, z2 = z2, Lpipe = Lpipe, delta_fin = delta_fin, hfin = hfin, sfin = sfin) annotation(Placement(visible = true, transformation(origin = {0, 0}, extent = {{-15, -15}, {15, 15}}, rotation = 0)));
    GF_HE HE3(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas, Toutgas = Toutgas, k_gamma_gas = k_gamma_gas, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow, Tinflow = Tinflow, Toutflow = Toutflow, numberOfTubeSections = numberOfTubeSections, numberPMCalcSections = numberPMCalcSections, numberOfFlueSections = numberOfFlueSections, Din = Din, delta = delta, s1 = s1, s2 = s2, zahod = 2, z1 = z1, z2 = z2, Lpipe = Lpipe, delta_fin = delta_fin, hfin = hfin, sfin = sfin) annotation(Placement(visible = true, transformation(origin = {50, 0}, extent = {{-15, -15}, {15, 15}}, rotation = 0)));
  equation
    connect(HE3.gasOut, gasvalve.inlet) annotation(Line(points = {{66.5, 7.5}, {66, 7.5}, {66, 36}, {28, 36}, {28, 58}, {28, 58}}, color = {0, 127, 255}));
    connect(HE2.gasOut, HE3.gasIn) annotation(Line(points = {{16.5, 7.5}, {34, 7.5}, {34, 6}, {34, 6}}, color = {0, 127, 255}));
    connect(HE1.gasOut, HE2.gasIn) annotation(Line(points = {{-33.5, 7.5}, {-16, 7.5}, {-16, 8}, {-16, 8}}, color = {0, 127, 255}));
    connect(sensgas1.outlet, HE1.gasIn) annotation(Line(points = {{-52, -60}, {-44, -60}, {-44, -20}, {-74, -20}, {-74, 8}, {-66, 8}, {-66, 8}}, color = {159, 159, 223}));
    connect(HE3.flowOut, flowvalve.inlet) annotation(Line(points = {{66.5, -7.5}, {76, -7.5}, {76, -72}, {24, -72}, {24, -90}, {24, -90}}, color = {0, 127, 255}));
    connect(HE2.flowOut, HE3.flowIn) annotation(Line(points = {{16.5, -7.5}, {34, -7.5}, {34, -8}, {34, -8}}, color = {0, 127, 255}));
    connect(HE1.flowOut, HE2.flowIn) annotation(Line(points = {{-33.5, -7.5}, {-16, -7.5}, {-16, -8}, {-16, -8}}, color = {0, 127, 255}));
    connect(sensflow1.outlet, HE1.flowIn) annotation(Line(points = {{-40, 60}, {-40, 60}, {-40, 40}, {-80, 40}, {-80, -8}, {-66, -8}, {-66, -8}}, color = {0, 0, 255}));
    connect(gasvalvestep.y, gasvalve.cmd) annotation(Line(points = {{29, 90}, {38, 90}, {38, 65}}, color = {0, 0, 127}));
    connect(sensflow2.outlet, sinkflow.flange) annotation(Line(points = {{66, -90}, {80, -90}, {80, -92}, {80, -92}}, color = {0, 0, 255}));
    ...
    connect(sensgas2.outlet, sinkgas.flange) annotation(Line(points = {{72, 58}, {80, 58}}, color = {159, 159, 223}));
    connect(gasvalve.outlet, sensgas2.inlet) annotation(Line(points = {{48, 58}, {60, 58}}, color = {159, 159, 223}));
    annotation(Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2})), Diagram(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2})), experiment(StartTime = 0, StopTime = 1000, Tolerance = 1e-06, Interval = 0.2));
  end testThreeGFHE;
...
//upStr.p = ps_start; //!!!!!!!!ЗАМЕНА
  ...
  model Circulation7
    ...
    //**
    replaceable package Medium_G = MyHRSG.ExhaustGas constrainedby Modelica.Media.Interfaces.PartialMedium;
    parameter SI.MassFlowRate wgas = 100 "Номинальный (и начальный) массовый расход газов";
    parameter SI.Pressure pgas = 3e3 "Начальное давление газов";
    parameter SI.Temperature Tingas = Tflow "Начальная входная температура газов";
    parameter SI.Temperature Toutgas = Tflow "Начальная выходная температура газов";
    parameter Real k_gamma_gas = 1 "Поправка к коэффициенту теплоотдачи со стороны газов";
    parameter Real Set_X[6] = Medium_G.reference_X "Состав дымовых газов";
    //**
    //***Исходные данные для водяной стороны
    //**
    replaceable package Medium_F = Modelica.Media.Water.WaterIF97_ph constrainedby Modelica.Media.Interfaces.PartialMedium;
    parameter Modelica.SIunits.MassFlowRate wflow = 40 "Номинальный массовый расход воды/пар";
    parameter Modelica.SIunits.Pressure pflow = 13e6 "Начальное давление потока вода/пар";
    parameter Medium_F.SaturationProperties sat_start = Medium_F.setSat_p(pflow);
    parameter SI.Temperature Tflow = Medium_F.saturationTemperature_sat(sat_start) "Начальная входная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_evo = Medium_F.bubbleEnthalpy(sat_start) "Начальная энтальпия входного потока вода/пар испарителя";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_sh = Medium_F.dewEnthalpy(sat_start) "Начальная энтальпия входного потока вода/пар пароперегревателя";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_eco = hflow_evo "Начальная энтальпия входного потока вода/пар пароперегревателя";
    //**
    //***Исходные данные по разбиению испарителя
    //**
    parameter Integer numberOfTubeSections_evo = 2 "Число участков разбиения трубы" annotation(Dialog(group = "Конструктивные характеристики"));
    parameter Integer numberPMCalcSections_evo = 2 "Число участков разбиения трубы входящих в один участок расчета процессов массообмена";
    parameter Integer numberOfFlueSections_evo = z2_evo "Число участков разбиения газохода" annotation(Dialog(group = "Конструктивные характеристики"));
    parameter Integer z2_evo = 2 "Число труб по ходу газов в данной поверхности нагрева";
    parameter Integer zahod_evo = 2 "заходность труб теплообменника";
    //**
    //***Исходные данные по разбиению пароперегревателя
    //**
    ...
    parameter Integer numberPMCalcSections_sh = 2 "Число участков разбиения трубы входящих в один участок расчета процессов массообмена";
    ...
    parameter Integer z2_sh = 1 "Число труб по ходу газов в данной поверхности нагрева";
    parameter Integer zahod_sh = 1 "заходность труб теплообменника";
    //**
    //***конструктивные характеристики поверхности нагрева
    //**
    parameter SI.Diameter Din = 0.038 "Внутренний диаметр трубок теплообменника";
    parameter SI.Length delta = 0.003 "Толщина стенки трубки теплообменника";
    parameter Modelica.SIunits.Length s1 = 82e-3 "Поперечный шаг";
    ...
    parameter Modelica.SIunits.Length sfin = 0.00404 "Шаг ребер, м";
    //**/
    parameter Modelica.SIunits.Length Din_set = 1.718 "Внутренний диаметр барабана";
    parameter Modelica.SIunits.Length L_set = 9 "Длина барабана";
    //***
    //Начальные значения
    parameter Modelica.SIunits.Length Hw_start_set = 0.5 "Начальное значение уровня воды в барабане";
    parameter Modelica.SIunits.MassFlowRate wflow_start = 0 "Стартовый массовый расход воды/пар";
    //Другие параметры
    parameter Modelica.SIunits.Time Tstab_set = 0;
    //**
    ThermoPower.Water.SensT sensflow1(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {-16, 0}, extent = {{-6, -6}, {6, 6}}, rotation = -90)));
    ThermoPower.Water.SourceMassFlow sourceMassFlow1(h = hflow_evo, p0 = pflow, use_in_h = false, use_in_w0 = false, w0 = 1.46679) annotation(Placement(visible = true, transformation(origin = {-90, 42}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ThermoPower.Water.SinkPressure sinkflow(p0 = 100000) annotation(Placement(visible = true, transformation(origin = {88, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ThermoPower.Water.SensT sensflow2(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {-2, 0}, extent = {{-6, 6}, {6, -6}}, rotation = 90)));
    MyHRSG.pipe pipe(setD = 100, setp = pflow, setT = Tflow, numberOfTubeSections = 2, Din = 0.2, Hpipe_in = 18.4, Hpipe_out = 0, Tstab = Tstab_set, delta = 0.003) annotation(Placement(visible = true, transformation(origin = {-20, -18}, extent = {{-4, -4}, {4, 4}}, rotation = -90)));
    MyHRSG.simpleDrum simpleDrum1(Din = Din_set, L = L_set, hout = 1400000, ps_start = pflow, Tstab = Tstab_set, delta = 0.02, Hw_start = Hw_start_set, sat_start = sat_start, t_start = Tflow) annotation(Placement(visible = true, transformation(origin = {-12, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    inner Modelica.Fluid.System system(allowFlowReversal = false, energyDynamics = Modelica.Fluid.Types.Dynamics.SteadyStateInitial, m_flow_start = 0, massDynamics = Modelica.Fluid.Types.Dynamics.SteadyStateInitial, momentumDynamics = Modelica.Fluid.Types.Dynamics.SteadyStateInitial) annotation(Placement(visible = true, transformation(origin = {90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ThermoPower.Gas.SourceMassFlow sourceGas(redeclare package Medium = Medium_G, T = Tingas, p0 = pgas, use_in_T = true, use_in_X = false, w0 = wgas) annotation(Placement(visible = true, transformation(origin = {70, -68}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    ThermoPower.Gas.SinkPressure sinkGas(redeclare package Medium = Medium_G, p0 = pgas, T = Tingas) annotation(Placement(visible = true, transformation(origin = {-58, -38}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    MyHRSG.GF_HE EVO(Din = Din, HRSG_type_set = MyHRSG.Choices.HRSG_type.verticalTop, Lpipe = Lpipe, Tinflow = Tflow, Tingas = Tingas, Toutflow = Tflow, Toutgas = Toutgas, delta = delta, delta_fin = delta_fin, hfin = hfin, k_gamma_gas = k_gamma_gas, numberOfFlueSections = numberOfFlueSections_evo, numberOfTubeSections = numberOfTubeSections_evo, numberPMCalcSections = numberPMCalcSections_evo, pflow = pflow, pgas = pgas, s1 = s1, s2 = s2, seth = hflow_evo, sfin = sfin, wflow = 100, wgas = wgas, z1 = z1, z2 = z2_evo, zahod = zahod_evo) annotation(Placement(visible = true, transformation(origin = {-12, -38}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    MyHRSG.GF_HE SH(Din = Din, HRSG_type_set = MyHRSG.Choices.HRSG_type.verticalBottom, Lpipe = Lpipe, Tinflow = Tflow, Tingas = Tingas, Toutflow = Tflow, Toutgas = Toutgas, delta = delta, delta_fin = delta_fin, hfin = hfin, k_gamma_gas = k_gamma_gas, numberOfFlueSections = numberOfFlueSections_sh, numberOfTubeSections = numberOfTubeSections_sh, numberPMCalcSections = numberPMCalcSections_sh, pflow = pflow, pgas = pgas, s1 = s1, s2 = s2, seth = hflow_sh, sfin = sfin, wflow = 0, wgas = wgas, z1 = z1, z2 = z2_sh, zahod = zahod_sh) annotation(Placement(visible = true, transformation(origin = {14, -38}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    MyHRSG.GF_HE ECO(Din = Din, HRSG_type_set = MyHRSG.Choices.HRSG_type.horizontalTop, Lpipe = Lpipe, Tinflow = Tflow, Tingas = Tingas, Toutflow = Tflow, Toutgas = Toutgas, delta = delta, delta_fin = delta_fin, hfin = hfin, k_gamma_gas = k_gamma_gas, numberOfFlueSections = numberOfFlueSections_sh, numberOfTubeSections = numberOfTubeSections_sh, numberPMCalcSections = numberPMCalcSections_sh, pflow = pflow, pgas = pgas, s1 = s1, s2 = s2, seth = hflow_eco, sfin = sfin, wflow = 0, wgas = wgas, z1 = z1, z2 = z2_sh, zahod = zahod_sh) annotation(Placement(visible = true, transformation(origin = {-30, -38}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Step step1(height = 100, offset = Tingas, startTime = 30) annotation(Placement(visible = true, transformation(origin = {62, -36}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Valves.ValveLinear valveLinear1(redeclare package Medium = Medium_F, dp_nominal = pflow - 100000, m_flow_nominal = 0.82927) annotation(Placement(visible = true, transformation(origin = {58, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Constant const(k = 1) annotation(Placement(visible = true, transformation(origin = {34, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(const.y, valveLinear1.opening) annotation(Line(points = {{45, 30}, {58, 30}, {58, 8}}, color = {0, 0, 127}));
    connect(valveLinear1.port_b, sinkflow.flange) annotation(Line(points = {{68, 0}, {78, 0}, {78, 0}, {78, 0}, {78, 0}, {78, 0}}, color = {0, 127, 255}));
    connect(SH.flowOut, valveLinear1.port_a) annotation(Line(points = {{17, -27}, {43, -27}, {43, 0}, {48, 0}}, color = {0, 127, 255}));
    connect(step1.y, sourceGas.in_T) annotation(Line(points = {{73, -36}, {75, -36}, {75, -36}, {77, -36}, {77, -58}, {70, -58}, {70, -63}}, color = {0, 0, 127}));
    connect(sourceMassFlow1.flange, ECO.flowIn) annotation(Line(points = {{-80, 42}, {-76.5, 42}, {-76.5, 42}, {-73, 42}, {-73, -27}, {-33, -27}}, color = {0, 0, 255}));
    connect(EVO.gasOut, ECO.gasIn) annotation(Line(points = {{-18.2, -38}, {-21.2, -38}, {-21.2, -38}, {-24.2, -38}, {-24.2, -38}, {-24.2, -38}}, color = {0, 127, 255}));
    connect(sinkGas.flange, ECO.gasOut) annotation(Line(points = {{-48, -38}, {-36, -38}, {-36, -38}, {-36, -38}}, color = {159, 159, 223}));
    connect(ECO.flowOut, simpleDrum1.fedWater) annotation(Line(points = {{-27, -27}, {-26, -27}, {-26, -27}, {-25, -27}, {-25, 29}, {-17, 29}, {-17, 29}, {-19, 29}, {-19, 29}}, color = {0, 127, 255}));
    connect(EVO.gasIn, SH.gasOut) annotation(Line(points = {{-5.8, -38}, {1.2, -38}, {1.2, -38}, {8.2, -38}, {8.2, -38}, {8.2, -38}}, color = {0, 127, 255}));
    connect(SH.gasIn, sourceGas.flange) annotation(Line(points = {{20.2, -38}, {28.2, -38}, {28.2, -38}, {36.2, -38}, {36.2, -68}, {60.2, -68}, {60.2, -68}}, color = {0, 127, 255}));
    connect(simpleDrum1.steam, SH.flowIn) annotation(Line(points = {{-5, 29}, {2, 29}, {2, 29}, {11, 29}, {11, -27}, {10, -27}, {10, -27}, {11, -27}}, color = {0, 127, 255}));
    connect(pipe.waterOut, EVO.flowIn) annotation(Line(points = {{-20, -23.2}, {-20, -23.2}, {-20, -23.2}, {-20, -23.2}, {-20, -25.2}, {-14, -25.2}, {-14, -25.2}}, color = {0, 127, 255}));
    connect(sensflow2.inlet, EVO.flowOut) annotation(Line(points = {{-4.4, -3.6}, {-4.4, -3.6}, {-4.4, -3.6}, {-4.4, -3.6}, {-4.4, -25.6}, {-8.4, -25.6}, {-8.4, -25.6}}, color = {0, 0, 255}));
    connect(simpleDrum1.FW_feedback, sourceMassFlow1.in_w0) annotation(Line(points = {{-1, 24.4}, {1, 24.4}, {1, 24.4}, {3, 24.4}, {3, 58.4}, {-93, 58.4}, {-93, 48.4}, {-94, 48.4}, {-94, 48.4}, {-95, 48.4}}, color = {0, 0, 127}));
    connect(simpleDrum1.downStr, sensflow1.inlet) annotation(Line(points = {{-19, 11}, {-19, 11}, {-19, 11}, {-17, 11}, {-17, 3}, {-19, 3}, {-19, 3}, {-19, 3}}, color = {0, 127, 255}));
    connect(simpleDrum1.upStr, sensflow2.outlet) annotation(Line(points = {{-5, 11}, {-5, 3}}, color = {0, 127, 255}));
    connect(sensflow1.outlet, pipe.waterIn) annotation(Line(points = {{-18.4, -3.6}, {-19.4, -3.6}, {-19.4, -3.6}, {-20.4, -3.6}, {-20.4, -11.6}, {-20.4, -11.6}, {-20.4, -11.6}, {-20.4, -11.6}}, color = {0, 0, 255}));
  initial equation
/*for i in 1:numberOfFlueSections_evo loop
      for j in 1:numberOfTubeSections_evo loop
        der(EVO.flowHE.h_flow_v[i, j]) = 0;
        der(EVO.flowHE.t_m[i, j]) = 0;
        der(EVO.gasHE.h_gas[i + 1, j]) = 0;
      end for;
    end for;
  ...
  //for j in 1:integer(numberOfTubeSections_sh / numberPMCalcSections_sh) loop
  //  SH.flowHE.D_flow_v[i, j] = 0;
  //end for;
    end for;
    for i in 1:numberOfFlueSections_sh loop
      for j in 1:numberOfTubeSections_sh loop
    ...
    der(simpleDrum1.ps) = 0;
    der(simpleDrum1.hw) = 0;
    annotation(Documentation(info = "<HTML>Испытывается модель испарителя</html>"), Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2})), Diagram(coordinateSystem(initialScale = 0.1)), experiment(StartTime = 0, StopTime = 100, Tolerance = 1e-06, Interval = 0.1), uses(Modelica(version = "3.2.1"), ThermoPower(version = "3.1")));
  end Circulation7;
  ...
  model onceThrough_v1 "Данные по ТЭЦ-12"
    //**
    //***Исходные данные для газовой стороны
    //**
    replaceable package Medium_G = MyHRSG.ExhaustGas constrainedby Modelica.Media.Interfaces.PartialMedium;
    parameter Modelica.SIunits.MassFlowRate wgas = 494.6 "Номинальный (и начальный) массовый расход газов (ТЭЦ-12 в номинале при 15С)";
    parameter Modelica.SIunits.Pressure pgas = 3e3 "Начальное давление газов";
    parameter Modelica.SIunits.Temperature Tingas = 307.8 + 273.15 "Начальная входная температура газов (по расчетам Березенца за ИВД при 15С в номинале ТЭЦ-12";
    parameter Modelica.SIunits.Temperature Toutgas = 235 + 273.15 "Начальная выходная температура газов";
    parameter Real k_gamma_gas = 1 "Поправка к коэффициенту теплоотдачи со стороны газов";
    parameter Real Set_X[6] = Medium_G.reference_X "Состав дымовых газов";
    //**
    //***Исходные данные для водяной стороны
    //**
    replaceable package Medium_F = Modelica.Media.Water.WaterIF97_ph constrainedby Modelica.Media.Interfaces.PartialMedium;
    parameter Modelica.SIunits.MassFlowRate wflow = 62.64 "Номинальный массовый расход воды/пар";
    parameter Modelica.SIunits.Pressure pflow = 82.73e5 "Начальное давление потока вода/пар";
    parameter Modelica.SIunits.Temperature Tinflow = 166.5 + 273.15 "Начальная входная температура потока воды/пар";
    parameter Modelica.SIunits.Temperature Toutflow = 299.6 + 273.15 "Начальная выходная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow = Medium_F.specificEnthalpy_pT(pflow, 0.5 * (Tinflow + Toutflow)) "Начальная энтальпия входного потока вода/пар";
    //**
    //***Исходные данные по разбиению
    //**
    parameter Integer numberOfTubeSections = 3 "Число участков разбиения трубы" annotation(Dialog(group = "Конструктивные характеристики"));
    ...
    //**
    parameter Modelica.SIunits.Diameter Din = 0.038 "Внутренний диаметр трубок теплообменника";
    parameter Modelica.SIunits.Length delta = 0.0035 "Толщина стенки трубки теплообменника";
    parameter Modelica.SIunits.Length s1 = 92.9e-3 "Поперечный шаг";
    parameter Modelica.SIunits.Length s2 = 79e-3 "Продольный шаг";
    parameter Integer zahod = 2 "заходность труб теплообменника";
    parameter Integer z1 = 78 "Число труб по ширине газохода";
    parameter Integer z2 = 16 "Число труб по ходу газов в теплообменнике";
    parameter Modelica.SIunits.Length Lpipe = 18.492 "Длина теплообменной трубки";
    ///Оребрение
    parameter Modelica.SIunits.Length delta_fin = 0.0008 "Средняя толщина ребра, м";
    parameter Modelica.SIunits.Length hfin = 0.017 "Высота ребра, м";
    parameter Modelica.SIunits.Length sfin = 3.636e-3 "Шаг ребер, м";
    //**
    //***
    //**
    ...
    Modelica.Fluid.Sources.MassFlowSource_T flowSource(redeclare package Medium = Medium_F, nPorts = 1, use_T_in = false, use_m_flow_in = false, m_flow = wflow, T = Tinflow) annotation(Placement(visible = true, transformation(origin = {-70, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary flowSink(redeclare package Medium = Medium_F, T = Toutflow, nPorts = 1, p = pflow, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {90, 30}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    GF_HE ECO(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas, Toutgas = Toutgas, k_gamma_gas = k_gamma_gas, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow, Tinflow = Tinflow, Toutflow = Toutflow, numberOfTubeSections = numberOfTubeSections, numberPMCalcSections = numberPMCalcSections, numberOfFlueSections = numberOfFlueSections, Din = Din, delta = delta, s1 = s1, s2 = s2, zahod = zahod, z1 = z1, z2 = z2, Lpipe = Lpipe, delta_fin = delta_fin, hfin = hfin, sfin = sfin, seth = hflow) annotation(Placement(visible = true, transformation(origin = {10, -12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T gasSource(redeclare package Medium = Medium_G, nPorts = 1, use_T_in = false, use_m_flow_in = false, m_flow = wgas, T = Tingas) annotation(Placement(visible = true, transformation(origin = {70, -30}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary gasSink(redeclare package Medium = Medium_G, T = Toutgas, nPorts = 1, p = pgas, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {-90, -30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(flowSink.ports[1], ECO.flowOut) annotation(Line(points = {{80, 30}, {14, 30}, {14, 0}, {14, 0}}, color = {0, 127, 255}));
    connect(flowSource.ports[1], ECO.flowIn) annotation(Line(points = {{-60, 30}, {6, 30}, {6, 0}, {6, 0}}, color = {0, 127, 255}));
    connect(gasSource.ports[1], ECO.gasIn) annotation(Line(points = {{60, -30}, {40, -30}, {40, -12}, {16, -12}, {16, -12}}, color = {0, 127, 255}));
    connect(gasSink.ports[1], ECO.gasOut) annotation(Line(points = {{-80, -30}, {-40, -30}, {-40, -12}, {4, -12}, {4, -12}}, color = {0, 127, 255}));
  initial equation
    for i in 1:numberOfFlueSections loop
      for j in 1:numberOfTubeSections loop
        der(ECO.flowHE.h_flow_v[i, j]) = 0;
...
//Опускной стояк
  ...
  model testSeparator
    ...
    //Исходные данные вода/пар
    package Medium_F = Modelica.Media.Water.WaterIF97_ph;
    parameter Modelica.SIunits.MassFlowRate wflow = 58 "Пусковой массовый расход воды на входе в сепаратор";
    parameter Modelica.SIunits.MassFlowRate wsteam = 23.66 "Расход пара на выходе из сепаратора";
    parameter Modelica.SIunits.Pressure pflow = 1.061e6 "Начальное давление потока вода/пар в сепараторе";
    parameter Modelica.SIunits.Pressure patm = 1.013e5 "Начальное давление потока вода/пар за клапанами";
    parameter Medium_F.SaturationProperties sat_start = Medium_F.setSat_p(pflow);
    parameter Modelica.SIunits.Temperature Tflow = 182.5 + 273.15 "Начальная входная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow = 1591.7e3 "Энтальпия на входе в сепаратор";
    MyHRSG.Separator separator(redeclare package Medium = Medium_F, Din = Dsep, L = Lsep, n_sep = n_sep_set, ps_start = pflow, delta = deltaSep, Hw_start = Hw_start_set, sat_start = sat_start, t_start = Medium_F.saturationTemperature_sat(sat_start)) annotation(Placement(visible = true, transformation(origin = {-24, -8}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary steamSink(redeclare package Medium = Medium_F, nPorts = 1, T = Tflow, p = patm, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {86, 14}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    inner Modelica.Fluid.System system annotation(Placement(visible = true, transformation(origin = {90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Valves.ValveLinear steamValve(redeclare package Medium = Medium_F, dp(start = pflow - patm), dp_nominal = pflow - patm, m_flow(start = wsteam), m_flow_nominal = wsteam) annotation(Placement(visible = true, transformation(origin = {36, 14}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Step steamStep(offset = 1, startTime = 1000) annotation(Placement(visible = true, transformation(origin = {6, 32}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_h flowSource(redeclare package Medium = Medium_F, m_flow = wflow, nPorts = 1, use_h_in = true) annotation(Placement(visible = true, transformation(origin = {-72, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_h waterSink(redeclare package Medium = Medium_F, h = hflow, nPorts = 1, use_m_flow_in = true) annotation(Placement(visible = true, transformation(origin = {70, -30}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Step step1(height = 1e5, offset = hflow, startTime = 100) annotation(Placement(visible = true, transformation(origin = {-50, 50}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  equation
    connect(step1.y, flowSource.h_in) annotation(Line(points = {{-62, 50}, {-92, 50}, {-92, 4}, {-84, 4}, {-84, 4}}, color = {0, 0, 127}));
    connect(separator.drainFeedback, waterSink.m_flow_in) annotation(Line(points = {{-18, -4}, {82, -4}, {82, -22}, {80, -22}}, color = {0, 0, 127}));
    connect(separator.downStr, waterSink.ports[1]) annotation(Line(points = {{-24, -18}, {-24, -18}, {-24, -30}, {60, -30}, {60, -30}}, color = {0, 127, 255}));
    connect(flowSource.ports[1], separator.fedWater) annotation(Line(points = {{-62, 0}, {-32, 0}, {-32, 0}, {-30, 0}}, color = {0, 127, 255}));
    connect(steamStep.y, steamValve.opening) annotation(Line(points = {{17, 32}, {26, 32}, {26, 32}, {35, 32}, {35, 22}, {35, 22}}, color = {0, 0, 127}));
    connect(steamValve.port_b, steamSink.ports[1]) annotation(Line(points = {{46, 14}, {61, 14}, {61, 14}, {76, 14}, {76, 14}, {76, 14}, {76, 14}, {76, 14}}, color = {0, 127, 255}));
    connect(separator.steam, steamValve.port_a) annotation(Line(points = {{-24, 3}, {-24, 8.5}, {-24, 8.5}, {-24, 14}, {1, 14}, {1, 14}, {26, 14}}, color = {0, 127, 255}));
  initial equation
    der(separator.ps) = 0;
    der(separator.hw) = 0;
    annotation(uses(Modelica(version = "3.2.1")));
  ...
  model onceThrough_v2
    ...
    parameter Modelica.SIunits.MassFlowRate wflow = 58 "Пусковой массовый расход воды на входе в сепаратор";
    parameter Modelica.SIunits.MassFlowRate wsteam = 23.66 "Расход пара на выходе из сепаратора";
    parameter Modelica.SIunits.Pressure pflow = 15.04e5 "Начальное давление потока вода/пар перед ECO";
    parameter Modelica.SIunits.Pressure patm = 1.013e5 "Начальное давление потока вода/пар за клапаном (турбиной)";
    parameter Medium_F.SaturationProperties sat_start = Medium_F.setSat_p(pflow);
    parameter Modelica.SIunits.Temperature Tinflow = 190 + 273.15 "Начальная входная температура потока воды/пар";
    parameter Modelica.SIunits.Temperature Toutflow = 200 + 273.15 "Начальная выходная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow = Medium_F.specificEnthalpy_pT(pflow, 0.5 * (Tinflow + Toutflow)) "Начальная энтальпия входного потока вода/пар";
    MyHRSG.Separator separator(redeclare package Medium = Medium_F, Din = Dsep, L = Lsep, n_sep = n_sep_set, ps_start = pflow, delta = deltaSep, Hw_start = Hw_start_set, sat_start = sat_start, t_start = Medium_F.saturationTemperature_sat(sat_start)) annotation(Placement(visible = true, transformation(origin = {10, -10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    //Исходные данные для газовой стороны
    replaceable package Medium_G = MyHRSG.ExhaustGas constrainedby Modelica.Media.Interfaces.PartialMedium;
    parameter Modelica.SIunits.MassFlowRate wgas = 1276.6 / 3.6 "Номинальный (и начальный) массовый расход газов (ТЭЦ-12 в номинале при 15С)";
    parameter Modelica.SIunits.Pressure pgas = 3e3 "Начальное давление газов";
    parameter Modelica.SIunits.Temperature Tingas = 200 + 273.15 "Начальная входная температура газов (по расчетам Березенца за ИВД при 15С в номинале ТЭЦ-12";
    parameter Modelica.SIunits.Temperature Toutgas = 180.7 + 273.15 "Начальная выходная температура газов";
    parameter Real k_gamma_gas = 1 "Поправка к коэффициенту теплоотдачи со стороны газов";
    parameter Real Set_X[6] = Medium_G.reference_X "Состав дымовых газов";
    Modelica.Fluid.Sources.FixedBoundary steamSink(redeclare package Medium = Medium_F, nPorts = 1, T = Toutflow, p = patm, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {86, 14}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    inner Modelica.Fluid.System system annotation(Placement(visible = true, transformation(origin = {90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Valves.ValveLinear steamValve(redeclare package Medium = Medium_F, dp(start = pflow - patm), dp_nominal = pflow - patm, m_flow(start = wsteam), m_flow_nominal = wsteam) annotation(Placement(visible = true, transformation(origin = {36, 14}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Step steamStep(offset = 1, startTime = 1000) annotation(Placement(visible = true, transformation(origin = {6, 32}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_h waterSink(redeclare package Medium = Medium_F, h = hflow, nPorts = 1, use_m_flow_in = true) annotation(Placement(visible = true, transformation(origin = {70, -30}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Step step1(height = 20, offset = Tinflow, startTime = 100) annotation(Placement(visible = true, transformation(origin = {-50, 50}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    MyHRSG.GF_HE ECO(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas, Toutgas = Tingas, k_gamma_gas = k_gamma_gas, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow, Tinflow = Tinflow, Toutflow = Tinflow, numberOfTubeSections = numberOfTubeSections_eco, numberPMCalcSections = numberPMCalcSections_eco, numberOfFlueSections = numberOfFlueSections_eco, Din = Din_eco, delta = delta_eco, s1 = s1_eco, s2 = s2_eco, zahod = zahod_eco, z1 = z1_eco, z2 = z2_eco, Lpipe = Lpipe, delta_fin = delta_fin_eco, hfin = hfin_eco, sfin = sfin_eco, seth = hflow) annotation(Placement(visible = true, transformation(origin = {-48, -10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T gasSource(redeclare package Medium = Medium_G, nPorts = 1, use_T_in = false, use_m_flow_in = false, m_flow = wgas, T = Tingas) annotation(Placement(visible = true, transformation(origin = {70, -70}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary gasSink(redeclare package Medium = Medium_G, T = Toutgas, nPorts = 1, p = pgas, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {-90, -70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    //GF_HE OTE1(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas, Toutgas = Tingas, k_gamma_gas = k_gamma_gas, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow, Tinflow = Tflow, Toutflow = Tflow, numberOfTubeSections = numberOfTubeSections_ote1, numberPMCalcSections = numberPMCalcSections_ote1, numberOfFlueSections = numberOfFlueSections_ote1, Din = Din_ote1, delta = delta_ote1, s1 = s1_ote1, s2 = s2_ote1, zahod = zahod_ote1, z1 = z1_ote1, z2 = z2_ote1, Lpipe = Lpipe, delta_fin = delta_fin_ote1, hfin = hfin_ote1, sfin = sfin_ote1) annotation(Placement(visible = true, transformation(origin = {-30, -10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    //GF_HE OTE2(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas, Toutgas = Tingas, k_gamma_gas = k_gamma_gas, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow, Tinflow = Tflow, Toutflow = Tflow, numberOfTubeSections = numberOfTubeSections_ote2, numberPMCalcSections = numberPMCalcSections_ote2, numberOfFlueSections = numberOfFlueSections_ote2, Din = Din_ote2, delta = delta_ote2, s1 = s1_ote2, s2 = s2_ote2, zahod = zahod_ote2, z1 = z1_ote2, z2 = z2_ote2, Lpipe = Lpipe, delta_fin = delta_fin_ote2, hfin = hfin_ote2, sfin = sfin_ote2) annotation(Placement(visible = true, transformation(origin = {-12, -10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T flowSource(redeclare package Medium = Medium_F, nPorts = 1, m_flow = wflow, use_T_in = true) annotation(Placement(visible = true, transformation(origin = {-78, 10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(ECO.flowOut, separator.fedWater) annotation(Line(points = {{-7.5, 1}, {3, 1}, {3, -3}}, color = {0, 127, 255}));
    connect(gasSource.ports[1], ECO.gasIn) annotation(Line(points = {{60, -70}, {-20, -70}, {-20, -10}, {-42, -10}, {-42, -10}}, color = {0, 127, 255}));
    connect(step1.y, flowSource.T_in) annotation(Line(points = {{-60, 50}, {-94, 50}, {-94, 14}, {-90, 14}, {-90, 14}}, color = {0, 0, 127}));
    connect(flowSource.ports[1], ECO.flowIn) annotation(Line(points = {{-68, 10}, {-52, 10}, {-52, 2}, {-52, 2}}, color = {0, 127, 255}));
//connect(OTE1.flowOut, OTE2.flowIn) annotation(Line(points = {{-26, 2}, {-16, 2}, {-16, 2}, {-16, 2}}, color = {0, 127, 255}));
//connect(ECO.flowOut, OTE1.flowIn) annotation(Line(points = {{-44, 2}, {-34, 2}, {-34, 2}, {-34, 2}}, color = {0, 127, 255}));
//connect(OTE1.gasOut, ECO.gasIn) annotation(Line(points = {{-36, -10}, {-42, -10}, {-42, -10}, {-42, -10}}, color = {0, 127, 255}));
//connect(OTE2.gasOut, OTE1.gasIn) annotation(Line(points = {{-18, -10}, {-24, -10}, {-24, -10}, {-24, -10}}, color = {0, 127, 255}));
//connect(gasSource.ports[1], OTE2.gasIn) annotation(Line(points = {{60, -70}, {0, -70}, {0, -10}, {-6, -10}, {-6, -10}}, color = {0, 127, 255}));
    connect(ECO.gasOut, gasSink.ports[1]) annotation(Line(points = {{-54, -10}, {-60, -10}, {-60, -70}, {-80, -70}}, color = {0, 127, 255}));
    connect(separator.steam, steamValve.port_a) annotation(Line(points = {{10, 1}, {10, 14}, {26, 14}}, color = {0, 127, 255}));
    connect(separator.downStr, waterSink.ports[1]) annotation(Line(points = {{10, -21}, {10, -30}, {60, -30}}, color = {0, 127, 255}));
    connect(separator.drainFeedback, waterSink.m_flow_in) annotation(Line(points = {{15, -7}, {82, -7}, {82, -22}, {80, -22}}, color = {0, 0, 127}));
    connect(steamStep.y, steamValve.opening) annotation(Line(points = {{17, 32}, {26, 32}, {26, 32}, {35, 32}, {35, 22}, {35, 22}}, color = {0, 0, 127}));
    connect(steamValve.port_b, steamSink.ports[1]) annotation(Line(points = {{46, 14}, {61, 14}, {61, 14}, {76, 14}, {76, 14}, {76, 14}, {76, 14}, {76, 14}}, color = {0, 127, 255}));
//connect(OTE2.flowOut, separator.fedWater) annotation(Line(points = {{-7.5, 1}, {3, 1}, {3, -3}}, color = {0, 127, 255}));
  initial equation
    der(separator.ps) = 0;
    der(separator.hw) = 0;
    annotation(uses(Modelica(version = "3.2.1")));
  ...
  model onceThrough_v3
    ...
    parameter Modelica.SIunits.Pressure pflow_eco = 15.04e5 "Начальное давление потока вода/пар перед ECO";
    ...
    parameter Integer numberOfFlueSections_ote1 = z2_ote1 "Число участков разбиения газохода" annotation(Dialog(group = "Конструктивные характеристики"));
    //Исходные данные вода/пар для экономайзера
    parameter Modelica.SIunits.Pressure pflow_ote1 = 12.57e5 "Начальное давление потока вода/пар перед ECO";
    parameter Modelica.SIunits.Temperature Tinflow_ote1 = 190 + 273.15 "Начальная входная температура потока воды/пар";
    parameter Modelica.SIunits.Temperature Toutflow_ote1 = 188 + 273.15 "Начальная выходная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_ote1_in = hflow_eco_out "Начальная энтальпия входного потока вода/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_ote1_out = 1.76425e6 "Начальная энтальпия выходного потока вода/пар";
    //Исходные данные для газовой стороны экономайзера
    parameter Modelica.SIunits.Temperature Tingas_ote1 = 197.3 + 273.15 "Начальная входная температура газов (по расчетам Березенца за ИВД при 15С в номинале ТЭЦ-12";
    parameter Modelica.SIunits.Temperature Toutgas_ote1 = 190.4 + 273.15 "Начальная выходная температура газов";
    ...
    Modelica.Fluid.Sources.FixedBoundary flowSink(redeclare package Medium = Medium_F, T = Toutflow_ote1, nPorts = 1, p = pflow_eco, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {90, 30}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    MyHRSG.GF_HE ECO(redeclare onlyFlowHEBoilLite flowHE, redeclare package Medium_G = Medium_G, HRSG_type_set = MyHRSG.Choices.HRSG_type.verticalTop, wgas = wgas, pgas = pgas, Tingas = Tingas_eco, Toutgas = Tingas_eco, k_gamma_gas = k_gamma_gas_eco, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_eco, pflow_out = pflow_eco, Tinflow = Tinflow_eco, Toutflow = Tinflow_eco, numberOfTubeSections = numberOfTubeSections_eco, numberPMCalcSections = numberPMCalcSections_eco, numberOfFlueSections = numberOfFlueSections_eco, Din = Din_eco, delta = delta_eco, s1 = s1_eco, s2 = s2_eco, zahod = zahod_eco, z1 = z1_eco, z2 = z2_eco, Lpipe = Lpipe, delta_fin = delta_fin_eco, hfin = hfin_eco, sfin = sfin_eco, seth_in = hflow_eco_in, seth_out = hflow_eco_out, m_flow_small = system.m_flow_small) annotation(Placement(visible = true, transformation(origin = {-14, -12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T gasSource(redeclare package Medium = Medium_G, nPorts = 1, use_T_in = false, use_m_flow_in = false, m_flow = wgas, T = Tingas_ote1) annotation(Placement(visible = true, transformation(origin = {70, -30}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    ...
    GF_HE OTE1(redeclare onlyFlowHEBoil_8 flowHE, redeclare package Medium_G = Medium_G, HRSG_type_set = MyHRSG.Choices.HRSG_type.verticalTop, wgas = wgas, pgas = pgas, Tingas = Tingas_ote1, Toutgas = Tingas_ote1, k_gamma_gas = k_gamma_gas_ote1, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_ote1, pflow_out = pflow_ote1, Tinflow = Tinflow_ote1, Toutflow = Tinflow_ote1, numberOfTubeSections = numberOfTubeSections_ote1, numberPMCalcSections = numberPMCalcSections_ote1, numberOfFlueSections = numberOfFlueSections_ote1, Din = Din_ote1, delta = delta_ote1, s1 = s1_ote1, s2 = s2_ote1, zahod = zahod_ote1, z1 = z1_ote1, z2 = z2_ote1, Lpipe = Lpipe, delta_fin = delta_fin_ote1, hfin = hfin_ote1, sfin = sfin_ote1, seth_in = hflow_ote1_in, seth_out = hflow_ote1_out, m_flow_small = system.m_flow_small) annotation(Placement(visible = true, transformation(origin = {16, -12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(OTE1.gasIn, gasSource.ports[1]) annotation(Line(points = {{22, -12}, {40, -12}, {40, -30}, {60, -30}, {60, -30}}, color = {0, 127, 255}));
    connect(OTE1.flowOut, flowSink.ports[1]) annotation(Line(points = {{20, 0}, {20, 0}, {20, 30}, {80, 30}, {80, 30}}, color = {0, 127, 255}));
    connect(OTE1.flowIn, ECO.flowOut) annotation(Line(points = {{12, 0}, {-10, 0}, {-10, 0}, {-10, 0}}, color = {0, 127, 255}));
    connect(ECO.gasIn, OTE1.gasOut) annotation(Line(points = {{-8, -12}, {10, -12}}, color = {0, 127, 255}));
  ...
  model onceThrough_v4
    package Medium_F = Modelica.Media.Water.WaterIF97_ph;
    parameter Modelica.SIunits.MassFlowRate wflow = 58 / 3.6 "Пусковой массовый расход воды на входе в сепаратор";
    parameter Modelica.SIunits.MassFlowRate wsteam = 23.66 / 3.6 "Расход пара на выходе из сепаратора";
    parameter Modelica.SIunits.Pressure patm = 1.013e5 "Начальное давление потока вода/пар за клапаном (турбиной)";
    replaceable package Medium_G = MyHRSG.ExhaustGas constrainedby Modelica.Media.Interfaces.PartialMedium;
    parameter Modelica.SIunits.MassFlowRate wgas = 1276.6 / 3.6 "Номинальный (и начальный) массовый расход газов ";
    parameter Modelica.SIunits.Pressure pgas = 3e3 "Начальное давление газов";
    ...
    parameter Modelica.SIunits.SpecificEnthalpy hflow_eco_out = Medium_F.specificEnthalpy_pT(pflow_eco, Toutflow_eco) "Начальная энтальпия выходного потока вода/пар";
    //Исходные данные для газовой стороны экономайзера
    parameter Modelica.SIunits.Temperature Tingas_eco = 190.4 + 273.15 "Начальная входная температура газов";
    parameter Modelica.SIunits.Temperature Toutgas_eco = 180.7 + 273.15 "Начальная выходная температура газов";
    parameter Real k_gamma_gas_eco = 500 "Поправка к коэффициенту теплоотдачи со стороны газов";
    //Исходные данные для прямоточного испарителя №1 (OTE1)
    parameter Modelica.SIunits.Diameter Din_ote1 = 0.038 "Внутренний диаметр трубок теплообменника";
    parameter Modelica.SIunits.Length delta_ote1 = 0.002 "Толщина стенки трубки теплообменника";
    parameter Modelica.SIunits.Length s1_ote1 = 91.09e-3 "Поперечный шаг";
    ...
    parameter Modelica.SIunits.SpecificEnthalpy hflow_ote1_out = 1.76425e6 "Начальная энтальпия выходного потока вода/пар";
    //Исходные данные для газовой стороны экономайзера
    parameter Modelica.SIunits.Temperature Tingas_ote1 = 197.3 + 273.15 "Начальная входная температура газов";
    parameter Modelica.SIunits.Temperature Toutgas_ote1 = 190.4 + 273.15 "Начальная выходная температура газов";
    parameter Real k_gamma_gas_ote1 = 500 "Поправка к коэффициенту теплоотдачи со стороны газов";
    //Исходные данные для прямоточного испарителя №2 (OTE2)
    parameter Modelica.SIunits.Diameter Din_ote2 = 0.038 "Внутренний диаметр трубок теплообменника";
    parameter Modelica.SIunits.Length delta_ote2 = 0.002 "Толщина стенки трубки теплообменника";
    parameter Modelica.SIunits.Length s1_ote2 = 91.09e-3 "Поперечный шаг";
    ...
    parameter Integer numberOfFlueSections_ote2 = z2_ote2 "Число участков разбиения газохода" annotation(Dialog(group = "Конструктивные характеристики"));
    //Исходные данные вода/пар для экономайзера
    parameter Modelica.SIunits.Pressure pflow_ote2 = 12.57e5 "Начальное давление потока вода/пар перед OTE2";
    parameter Modelica.SIunits.Temperature Tinflow_ote2 = 190 + 273.15 "Начальная входная температура потока воды/пар";
    parameter Modelica.SIunits.Temperature Toutflow_ote2 = 188 + 273.15 "Начальная выходная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_ote2_in = hflow_ote1_out "Начальная энтальпия входного потока вода/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_ote2_out = 2.49228e6 "Начальная энтальпия выходного потока вода/пар";
    //Исходные данные для газовой стороны экономайзера
    parameter Modelica.SIunits.Temperature Tingas_ote2 = 300 + 273.15 "Начальная входная температура газов (по расчетам Березенца за ИВД при 15С в номинале ТЭЦ-12";
    parameter Modelica.SIunits.Temperature Toutgas_ote2 = 190.4 + 273.15 "Начальная выходная температура газов";
    parameter Real k_gamma_gas_ote2 = 500 "Поправка к коэффициенту теплоотдачи со стороны газов";
    inner Modelica.Fluid.System system(allowFlowReversal = false) annotation(Placement(visible = true, transformation(origin = {90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T flowSource(redeclare package Medium = Medium_F, T = Tinflow_eco, m_flow = wflow, nPorts = 1, use_T_in = false, use_m_flow_in = true) annotation(Placement(visible = true, transformation(origin = {-70, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary flowSink(redeclare package Medium = Medium_F, T = Toutflow_ote1, nPorts = 1, p = pflow_eco, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {90, 30}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    MyHRSG.GF_HE ECO(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_eco, Toutgas = Tingas_eco, k_gamma_gas = k_gamma_gas_eco, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow_eco, Tinflow = Tinflow_eco, Toutflow = Tinflow_eco, numberOfTubeSections = numberOfTubeSections_eco, numberPMCalcSections = numberPMCalcSections_eco, numberOfFlueSections = numberOfFlueSections_eco, Din = Din_eco, delta = delta_eco, s1 = s1_eco, s2 = s2_eco, zahod = zahod_eco, z1 = z1_eco, z2 = z2_eco, Lpipe = Lpipe, delta_fin = delta_fin_eco, hfin = hfin_eco, sfin = sfin_eco, seth_in = hflow_eco_in, seth_out = hflow_eco_out) annotation(Placement(visible = true, transformation(origin = {-26, -12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T gasSource(redeclare package Medium = Medium_G, nPorts = 1, use_T_in = false, use_m_flow_in = false, m_flow = wgas, T = Tingas_ote2) annotation(Placement(visible = true, transformation(origin = {70, -30}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary gasSink(redeclare package Medium = Medium_G, T = Toutgas_eco, nPorts = 1, p = pgas, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {-90, -30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    MyHRSG.GF_HE OTE1(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_ote1, Toutgas = Tingas_ote1, k_gamma_gas = k_gamma_gas_ote1, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow_ote1, Tinflow = Tinflow_ote1, Toutflow = Tinflow_ote1, numberOfTubeSections = numberOfTubeSections_ote1, numberPMCalcSections = numberPMCalcSections_ote1, numberOfFlueSections = numberOfFlueSections_ote1, Din = Din_ote1, delta = delta_ote1, s1 = s1_ote1, s2 = s2_ote1, zahod = zahod_ote1, z1 = z1_ote1, z2 = z2_ote1, Lpipe = Lpipe, delta_fin = delta_fin_ote1, hfin = hfin_ote1, sfin = sfin_ote1, seth_in = hflow_ote1_in, seth_out = hflow_ote1_out) annotation(Placement(visible = true, transformation(origin = {-2, -12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    GF_HE OTE2(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_ote2, Toutgas = Tingas_ote2, k_gamma_gas = k_gamma_gas_ote2, redeclare package Medium_F = Medium_F, wflow = wflow, pflow = pflow_ote2, Tinflow = Tinflow_ote2, Toutflow = Tinflow_ote2, numberOfTubeSections = numberOfTubeSections_ote2, numberPMCalcSections = numberPMCalcSections_ote2, numberOfFlueSections = numberOfFlueSections_ote2, Din = Din_ote2, delta = delta_ote2, s1 = s1_ote2, s2 = s2_ote2, zahod = zahod_ote2, z1 = z1_ote2, z2 = z2_ote2, Lpipe = Lpipe, delta_fin = delta_fin_ote2, hfin = hfin_ote2, sfin = sfin_ote2, seth_in = hflow_ote2_in, seth_out = hflow_ote2_out) annotation(Placement(visible = true, transformation(origin = {22, -12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sensors.TemperatureTwoPort temperature1(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {-18, 6}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    Modelica.Fluid.Sensors.TemperatureTwoPort temperature2(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {10, 6}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    Modelica.Fluid.Sensors.TemperatureTwoPort temperature3(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {58, 30}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    Modelica.Blocks.Sources.Ramp ramp1(duration = 5, height = 1, offset = wflow, startTime = 10) annotation(Placement(visible = true, transformation(origin = {-72, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(ramp1.y, flowSource.m_flow_in) annotation(Line(points = {{-60, 70}, {-54, 70}, {-54, 52}, {-92, 52}, {-92, 38}, {-80, 38}, {-80, 38}}, color = {0, 0, 127}));
    connect(temperature3.port_a, OTE2.flowOut) annotation(Line(points = {{54, 30}, {26, 30}, {26, 0}, {26, 0}}, color = {0, 127, 255}));
    connect(flowSink.ports[1], temperature3.port_b) annotation(Line(points = {{80, 30}, {62, 30}, {62, 30}, {62, 30}}, color = {0, 127, 255}));
    connect(OTE1.flowOut, temperature2.port_a) annotation(Line(points = {{2, -1}, {2, 6}, {6, 6}}, color = {0, 127, 255}));
    connect(ECO.gasIn, OTE1.gasOut) annotation(Line(points = {{-20, -12}, {-8, -12}}, color = {0, 127, 255}));
    connect(OTE1.gasIn, OTE2.gasOut) annotation(Line(points = {{4, -12}, {16, -12}}, color = {0, 127, 255}));
    connect(temperature1.port_b, OTE1.flowIn) annotation(Line(points = {{-14, 6}, {-6, 6}, {-6, -1}}, color = {0, 127, 255}));
    connect(temperature2.port_b, OTE2.flowIn) annotation(Line(points = {{14, 6}, {18, 6}, {18, 0}, {18, 0}}, color = {0, 127, 255}));
    connect(ECO.flowOut, temperature1.port_a) annotation(Line(points = {{-22, 0}, {-22, 0}, {-22, 6}, {-22, 6}}, color = {0, 127, 255}));
    connect(OTE2.gasIn, gasSource.ports[1]) annotation(Line(points = {{28, -12}, {36, -12}, {36, -30}, {60, -30}, {60, -30}}, color = {0, 127, 255}));
    connect(flowSource.ports[1], ECO.flowIn) annotation(Line(points = {{-60, 30}, {-30, 30}, {-30, -1}}, color = {0, 127, 255}));
    connect(gasSink.ports[1], ECO.gasOut) annotation(Line(points = {{-80, -30}, {-40, -30}, {-40, -12}, {-32, -12}}, color = {0, 127, 255}));
  initial equation
    for i in 1:numberOfFlueSections_eco loop
      for j in 1:numberOfTubeSections_eco loop
        der(ECO.flowHE.h_flow_n[i, j + 1]) = 0;
  ...
  model onceThrough_v5 "Прямоточный контур КУ"
    package Medium_F = Modelica.Media.Water.WaterIF97_ph;
    parameter Modelica.SIunits.MassFlowRate wflow = 58 / 3.6 "Пусковой массовый расход воды на входе в сепаратор";
    ...
    inner Modelica.Fluid.System system(allowFlowReversal = false) annotation(Placement(visible = true, transformation(origin = {90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T flowSource(redeclare package Medium = Medium_F, nPorts = 1, use_T_in = false, use_m_flow_in = false, m_flow = wflow, T = Tinflow_eco) annotation(Placement(visible = true, transformation(origin = {-70, 54}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary flowSink(redeclare package Medium = Medium_F, T = Toutflow_ote1, nPorts = 1, p = pflow_eco, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {90, 54}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    MyHRSG.GF_HE ECO(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_eco, Toutgas = Tingas_eco, k_gamma_gas = k_gamma_gas_eco, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_eco, pflow_out = pflow_eco, Tinflow = Tinflow_eco, Toutflow = Tinflow_eco, numberOfTubeSections = numberOfTubeSections_eco, numberPMCalcSections = numberPMCalcSections_eco, numberOfFlueSections = numberOfFlueSections_eco, Din = Din_eco, delta = delta_eco, s1 = s1_eco, s2 = s2_eco, zahod = zahod_eco, z1 = z1_eco, z2 = z2_eco, Lpipe = Lpipe, delta_fin = delta_fin_eco, hfin = hfin_eco, sfin = sfin_eco, seth_in = hflow_eco_in, seth_out = hflow_eco_out, setTm = setTm_eco) annotation(Placement(visible = true, transformation(origin = {-26, 12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    MyHRSG.GF_HE OTE1(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_ote1, Toutgas = Tingas_ote1, k_gamma_gas = k_gamma_gas_ote1, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_ote1, pflow_out = pflow_ote1, Tinflow = Tinflow_ote1, Toutflow = Tinflow_ote1, numberOfTubeSections = numberOfTubeSections_ote1, numberPMCalcSections = numberPMCalcSections_ote1, numberOfFlueSections = numberOfFlueSections_ote1, Din = Din_ote1, delta = delta_ote1, s1 = s1_ote1, s2 = s2_ote1, zahod = zahod_ote1, z1 = z1_ote1, z2 = z2_ote1, Lpipe = Lpipe, delta_fin = delta_fin_ote1, hfin = hfin_ote1, sfin = sfin_ote1, seth_in = hflow_ote1_in, seth_out = hflow_ote1_out, setTm = setTm_ote1) annotation(Placement(visible = true, transformation(origin = {-2, 12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    MyHRSG.GF_HE OTE2(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_ote2, Toutgas = Tingas_ote2, k_gamma_gas = k_gamma_gas_ote2, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_ote2, pflow_out = pflow_ote2, Tinflow = Tinflow_ote2, Toutflow = Tinflow_ote2, numberOfTubeSections = numberOfTubeSections_ote2, numberPMCalcSections = numberPMCalcSections_ote2, numberOfFlueSections = numberOfFlueSections_ote2, Din = Din_ote2, delta = delta_ote2, s1 = s1_ote2, s2 = s2_ote2, zahod = zahod_ote2, z1 = z1_ote2, z2 = z2_ote2, Lpipe = Lpipe, delta_fin = delta_fin_ote2, hfin = hfin_ote2, sfin = sfin_ote2, seth_in = hflow_ote2_in, seth_out = hflow_ote2_out, setTm = setTm_ote2) annotation(Placement(visible = true, transformation(origin = {22, 12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sensors.TemperatureTwoPort temperature1(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {-18, 30}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    ...
    MyHRSG.Separator separator(redeclare package Medium = Medium_F, Din = Dsep, L = Lsep, n_sep = n_sep_set, ps_start = pflow_ote2, delta = deltaSep, Hw_start = Hw_start_set, sat_start = sat_start, t_start = Medium_F.saturationTemperature_sat(sat_start)) annotation(Placement(visible = true, transformation(origin = {50, 34}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    Modelica.Blocks.Sources.Ramp rampGasFlow(duration = 20, height = -100, offset = wgas, startTime = 10) annotation(Placement(visible = true, transformation(origin = {70, -66}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Ramp rampGasTemp(duration = 40, height = -300, offset = Tingas_ote2, startTime = 10) annotation(Placement(visible = true, transformation(origin = {70, -34}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(gasSource.T_in, rampGasTemp.y) annotation(Line(points = {{82, -2}, {92, -2}, {92, -34}, {82, -34}, {82, -34}}, color = {0, 0, 127}));
    connect(rampGasFlow.y, gasSource.m_flow_in) annotation(Line(points = {{82, -66}, {98, -66}, {98, 2}, {80, 2}, {80, 2}}, color = {0, 0, 127}));
    connect(flowSource.ports[1], ECO.flowIn) annotation(Line(points = {{-60, 54}, {-30, 54}, {-30, 23}}, color = {0, 127, 255}));
    connect(separator.drainFeedback, waterSink.m_flow_in) annotation(Line(points = {{55, 37}, {89, 37}, {89, 29}, {79, 29}}, color = {0, 0, 127}));
    connect(waterSink.ports[1], separator.downStr) annotation(Line(points = {{60, 22}, {55, 22}, {55, 24}, {50, 24}, {50, 22}, {50, 22}}, color = {0, 127, 255}));
    connect(OTE2.flowOut, separator.fedWater) annotation(Line(points = {{26.2, 23}, {26.2, 23}, {26.2, 25}, {26.2, 25}, {26.2, 41}, {44.2, 41}, {44.2, 41}}, color = {0, 127, 255}));
    connect(separator.steam, flowSink.ports[1]) annotation(Line(points = {{50, 45}, {50, 45}, {50, 45}, {50, 45}, {50, 53}, {80, 53}, {80, 53}}, color = {0, 127, 255}));
    connect(temperature2.port_b, OTE2.flowIn) annotation(Line(points = {{14, 30}, {18, 30}, {18, 26}, {18, 26}, {18, 24}, {18, 24}}, color = {0, 127, 255}));
    connect(OTE1.flowOut, temperature2.port_a) annotation(Line(points = {{2.2, 23}, {2.2, 30}, {6.2, 30}}, color = {0, 127, 255}));
    connect(ECO.flowOut, temperature1.port_a) annotation(Line(points = {{-21.8, 23}, {-21.8, 23}, {-21.8, 23}, {-21.8, 23}, {-21.8, 31}, {-21.8, 31}, {-21.8, 29}, {-21.8, 29}}, color = {0, 127, 255}));
    connect(temperature1.port_b, OTE1.flowIn) annotation(Line(points = {{-14, 30}, {-6, 30}, {-6, 23}}, color = {0, 127, 255}));
    connect(OTE2.gasIn, gasSource.ports[1]) annotation(Line(points = {{28.2, 12}, {36.2, 12}, {36.2, -4}, {60.2, -4}, {60.2, -6}}, color = {0, 127, 255}));
    connect(OTE1.gasIn, OTE2.gasOut) annotation(Line(points = {{4.2, 12}, {16.2, 12}}, color = {0, 127, 255}));
    connect(ECO.gasIn, OTE1.gasOut) annotation(Line(points = {{-19.8, 12}, {-7.8, 12}}, color = {0, 127, 255}));
    connect(gasSink.ports[1], ECO.gasOut) annotation(Line(points = {{-80, -6}, {-40, -6}, {-40, 12}, {-32, 12}}, color = {0, 127, 255}));
  initial equation
    for i in 1:numberOfFlueSections_eco loop
      for j in 1:numberOfTubeSections_eco loop
        der(ECO.flowHE.h_flow_n[i, j + 1]) = 0;
...
Параметры взяты из модели прямоточного котла для ГТЭ-110 в Thermoflex ''ВертрПрямКУсГТЭ110_OD''
  ...
  model onceThrough_v6 "Прямоточный контур КУ"
    package Medium_F = Modelica.Media.Water.WaterIF97_ph;
    parameter Modelica.SIunits.MassFlowRate wflow = 58 / 3.6 "Пусковой массовый расход воды на входе в сепаратор";
    parameter Modelica.SIunits.MassFlowRate wsteam = 23.66 / 3.6 "Расход пара на выходе из сепаратора";
    parameter Modelica.SIunits.Pressure patm = 1.013e5 "Начальное давление потока вода/пар за клапаном (турбиной)";
    ...
    parameter Modelica.SIunits.MassFlowRate wgas = 1276.6 / 3.6 "Номинальный (и начальный) массовый расход газов ";
    parameter Modelica.SIunits.Pressure pgas = 3e3 "Начальное давление газов";
    parameter Medium_F.SaturationProperties sat_start = Medium_F.setSat_p(pflow_sh);
    //Исходные данные для сепаратора
    parameter Modelica.SIunits.Length Dsep = 0.348 "Внутренний диаметр сепаратор";
    parameter Modelica.SIunits.Length Lsep = 5 "Длина (высота) сепаратора";
    parameter Modelica.SIunits.Length deltaSep = 0.04 "Толщина стенки сепаратора";
    ...
    parameter Integer numberOfFlueSections_eco = z2_eco "Число участков разбиения газохода" annotation(Dialog(group = "Конструктивные характеристики"));
    //Исходные данные вода/пар для экономайзера
    parameter Modelica.SIunits.Pressure pflow_eco = 17.65e5 "Начальное давление потока вода/пар перед ECO";
    parameter Modelica.SIunits.Temperature Tinflow_eco = 140 + 273.15 "Начальная входная температура потока воды/пар";
    parameter Modelica.SIunits.Temperature Toutflow_eco = 206 + 273.15 "Начальная выходная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_eco_in = Medium_F.specificEnthalpy_pT(pflow_eco, Tinflow_eco) "Начальная энтальпия входного потока вода/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_eco_out = 950907 "Начальная энтальпия выходного потока вода/пар";
    //Исходные данные для газовой стороны экономайзера
    parameter Modelica.SIunits.Temperature Tingas_eco = 190.4 + 273.15 "Начальная входная температура газов";
    parameter Modelica.SIunits.Temperature Toutgas_eco = 180.7 + 273.15 "Начальная выходная температура газов";
    parameter Real k_gamma_gas_eco = 1 "Поправка к коэффициенту теплоотдачи со стороны газов";
    ...
    parameter Integer numberOfFlueSections_ote1 = z2_ote1 "Число участков разбиения газохода" annotation(Dialog(group = "Конструктивные характеристики"));
    //Исходные данные вода/пар для экономайзера
    parameter Modelica.SIunits.Pressure pflow_ote1 = 17.43e5 "Начальное давление потока вода/пар перед ECO";
    parameter Modelica.SIunits.Temperature Tinflow_ote1 = 190 + 273.15 "Начальная входная температура потока воды/пар";
    parameter Modelica.SIunits.Temperature Toutflow_ote1 = 188 + 273.15 "Начальная выходная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_ote1_in = 950907 "Начальная энтальпия входного потока вода/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_ote1_out = 1.76425e6 "Начальная энтальпия выходного потока вода/пар";
    //Исходные данные для газовой стороны экономайзера
    parameter Modelica.SIunits.Temperature Tingas_ote1 = 197.3 + 273.15 "Начальная входная температура газов";
    parameter Modelica.SIunits.Temperature Toutgas_ote1 = 190.4 + 273.15 "Начальная выходная температура газов";
    ...
    parameter Integer numberOfFlueSections_ote2 = z2_ote2 "Число участков разбиения газохода" annotation(Dialog(group = "Конструктивные характеристики"));
    //Исходные данные вода/пар для испарителя №2
    parameter Modelica.SIunits.Pressure pflow_ote2 = 11.92e5 "Начальное давление потока вода/пар перед OTE2";
    parameter Modelica.SIunits.Temperature Tinflow_ote2 = 190 + 273.15 "Начальная входная температура потока воды/пар";
    parameter Modelica.SIunits.Temperature Toutflow_ote2 = 188 + 273.15 "Начальная выходная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_ote2_in = 1.76425e6 "Начальная энтальпия входного потока вода/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_ote2_out = 2.49228e6 "Начальная энтальпия выходного потока вода/пар";
    //Исходные данные для газовой стороны испарителя №2
    parameter Modelica.SIunits.Temperature Tingas_ote2 = 300 + 273.15 "Начальная входная температура газов (по расчетам Березенца за ИВД при 15С в номинале ТЭЦ-12";
    parameter Modelica.SIunits.Temperature Toutgas_ote2 = 190.4 + 273.15 "Начальная выходная температура газов";
    ...
    parameter Modelica.SIunits.Pressure pflow_sh = 10.01e5 "Начальное давление потока вода/пар перед SH";
    parameter Modelica.SIunits.Temperature Tinflow_sh = 191 + 273.15 "Начальная входная температура потока воды/пар";
    parameter Modelica.SIunits.Temperature Toutflow_sh = 290.9 + 273.15 "Начальная выходная температура потока воды/пар";
    parameter Modelica.SIunits.SpecificEnthalpy hflow_sh_in = 2.78362e6 "Начальная энтальпия входного потока вода/пар";
    ...
    //Исходные данные для газовой стороны испарителя №2
    parameter Modelica.SIunits.Temperature Tingas_sh = 350 + 273.15 "Начальная входная температура газов";
    parameter Modelica.SIunits.Temperature Toutgas_sh = 300 + 273.15 "Начальная выходная температура газов";
    parameter Real k_gamma_gas_sh = 1 "Поправка к коэффициенту теплоотдачи со стороны газов";
    inner Modelica.Fluid.System system(allowFlowReversal = false) annotation(Placement(visible = true, transformation(origin = {90, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T flowSource(redeclare package Medium = Medium_F, nPorts = 1, use_T_in = false, use_m_flow_in = false, m_flow = wflow, T = Tinflow_eco) annotation(Placement(visible = true, transformation(origin = {-86, 22}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary flowSink(redeclare package Medium = Medium_F, T = Toutflow_ote1, nPorts = 1, p = 100000, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {88, 30}, extent = {{6, -6}, {-6, 6}}, rotation = 0)));
    MyHRSG.GF_HE ECO(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_eco, Toutgas = Tingas_eco, k_gamma_gas = k_gamma_gas_eco, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_eco, pflow_out = pflow_eco, Tinflow = Tinflow_eco, Toutflow = Tinflow_eco, numberOfTubeSections = numberOfTubeSections_eco, numberPMCalcSections = numberPMCalcSections_eco, numberOfFlueSections = numberOfFlueSections_eco, Din = Din_eco, delta = delta_eco, s1 = s1_eco, s2 = s2_eco, zahod = zahod_eco, z1 = z1_eco, z2 = z2_eco, Lpipe = Lpipe, delta_fin = delta_fin_eco, hfin = hfin_eco, sfin = sfin_eco, seth_in = hflow_eco_in, seth_out = hflow_eco_out) annotation(Placement(visible = true, transformation(origin = {-52, -14}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_T gasSource(redeclare package Medium = Medium_G, nPorts = 1, use_T_in = true, use_m_flow_in = true) annotation(Placement(visible = true, transformation(origin = {70, -14}, extent = {{6, -6}, {-6, 6}}, rotation = 0)));
    Modelica.Fluid.Sources.FixedBoundary gasSink(redeclare package Medium = Medium_G, T = Toutgas_eco, nPorts = 1, p = pgas, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {-92, -14}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
    MyHRSG.GF_HE OTE1(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_ote1, Toutgas = Tingas_ote1, k_gamma_gas = k_gamma_gas_ote1, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_ote1, pflow_out = pflow_ote1, Tinflow = Tinflow_ote1, Toutflow = Tinflow_ote1, numberOfTubeSections = numberOfTubeSections_ote1, numberPMCalcSections = numberPMCalcSections_ote1, numberOfFlueSections = numberOfFlueSections_ote1, Din = Din_ote1, delta = delta_ote1, s1 = s1_ote1, s2 = s2_ote1, zahod = zahod_ote1, z1 = z1_ote1, z2 = z2_ote1, Lpipe = Lpipe, delta_fin = delta_fin_ote1, hfin = hfin_ote1, sfin = sfin_ote1, seth_in = hflow_ote1_in, seth_out = hflow_ote1_out) annotation(Placement(visible = true, transformation(origin = {-28, -14}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    MyHRSG.GF_HE OTE2(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_ote2, Toutgas = Tingas_ote2, k_gamma_gas = k_gamma_gas_ote2, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_ote2, pflow_out = pflow_ote2, Tinflow = Tinflow_ote2, Toutflow = Tinflow_ote2, numberOfTubeSections = numberOfTubeSections_ote2, numberPMCalcSections = numberPMCalcSections_ote2, numberOfFlueSections = numberOfFlueSections_ote2, Din = Din_ote2, delta = delta_ote2, s1 = s1_ote2, s2 = s2_ote2, zahod = zahod_ote2, z1 = z1_ote2, z2 = z2_ote2, Lpipe = Lpipe, delta_fin = delta_fin_ote2, hfin = hfin_ote2, sfin = sfin_ote2, seth_in = hflow_ote2_in, seth_out = hflow_ote2_out) annotation(Placement(visible = true, transformation(origin = {-4, -14}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sensors.TemperatureTwoPort temperature1(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {-44, 4}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    Modelica.Fluid.Sensors.TemperatureTwoPort temperature2(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {-16, 4}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    MyHRSG.Separator separator(redeclare package Medium = Medium_F, Din = Dsep, L = Lsep, n_sep = n_sep_set, ps_start = pflow_sh, delta = deltaSep, Hw_start = Hw_start_set, sat_start = sat_start, t_start = Medium_F.saturationTemperature_sat(sat_start)) annotation(Placement(visible = true, transformation(origin = {14, 10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sources.MassFlowSource_h waterSink(redeclare package Medium = Medium_F, h = hflow_eco_out, nPorts = 1, use_m_flow_in = true) annotation(Placement(visible = true, transformation(origin = {70, 6}, extent = {{6, -6}, {-6, 6}}, rotation = 0)));
    Modelica.Blocks.Sources.Step stepGasTemp(height = 100, offset = Tingas_ote2, startTime = 10) annotation(Placement(visible = true, transformation(origin = {67, -31}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    Modelica.Blocks.Sources.Ramp rampGasFlow(duration = 20, height = 50, offset = wgas, startTime = 10) annotation(Placement(visible = true, transformation(origin = {67, -53}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
    MyHRSG.GF_HE SH(redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_sh, Toutgas = Tingas_sh, k_gamma_gas = k_gamma_gas_sh, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_sh, pflow_out = pflow_sh, Tinflow = Tinflow_sh, Toutflow = Tinflow_sh, numberOfTubeSections = numberOfTubeSections_sh, numberPMCalcSections = numberPMCalcSections_sh, numberOfFlueSections = numberOfFlueSections_sh, Din = Dout_sh - 2 * delta_sh, delta = delta_sh, s1 = s1_sh, s2 = s2_sh, zahod = zahod_sh, z1 = z1_sh, z2 = z2_sh, Lpipe = Lpipe, delta_fin = delta_fin_sh, hfin = hfin_sh, sfin = sfin_sh, seth_in = hflow_sh_in, seth_out = hflow_sh_out) annotation(Placement(visible = true, transformation(origin = {30, -14}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Fluid.Sensors.TemperatureTwoPort temperature3(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {0, 18}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    Modelica.Fluid.Sensors.TemperatureTwoPort temperature4(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {20, 32}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    Modelica.Fluid.Sensors.TemperatureTwoPort temperature5(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {50, 30}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    Modelica.Fluid.Sensors.TemperatureTwoPort temperature6(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {50, 0}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    Modelica.Fluid.Valves.ValveLinear valve(redeclare package Medium = Medium_F, dp_nominal = pflow_sh - 100000, m_flow_nominal = wflow) annotation(Placement(visible = true, transformation(origin = {66, 30}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
    Modelica.Blocks.Sources.Constant const(k = 1) annotation(Placement(visible = true, transformation(origin = {53, 49}, extent = {{-7, -7}, {7, 7}}, rotation = 0)));
  equation
    connect(temperature6.port_a, separator.downStr) annotation(Line(points = {{46, 0}, {42, 0}, {42, -26}, {14, -26}, {14, 0}}, color = {0, 127, 255}));
    connect(temperature6.port_b, waterSink.ports[1]) annotation(Line(points = {{54, 0}, {64, 0}, {64, 6}}, color = {0, 127, 255}));
    connect(const.y, valve.opening) annotation(Line(points = {{60.7, 49}, {63.2, 49}, {63.2, 51}, {65.7, 51}, {65.7, 41.5}, {65.7, 41.5}, {65.7, 34}}, color = {0, 0, 127}));
    connect(temperature5.port_b, valve.port_a) annotation(Line(points = {{54, 30}, {62, 30}, {62, 30}, {62, 30}}, color = {0, 127, 255}));
    connect(valve.port_b, flowSink.ports[1]) annotation(Line(points = {{70, 30}, {82, 30}}, color = {0, 127, 255}));
    connect(SH.flowOut, temperature5.port_a) annotation(Line(points = {{34.2, -3}, {34.2, -3}, {34.2, -1}, {34.2, -1}, {34.2, 31}, {46.2, 31}, {46.2, 29}, {46.2, 29}, {46.2, 29}}, color = {0, 127, 255}));
    connect(separator.steam, temperature4.port_a) annotation(Line(points = {{14, 21}, {14, 21}, {14, 21}, {14, 21}, {14, 31}, {16, 31}, {16, 31}}, color = {0, 127, 255}));
    connect(temperature4.port_b, SH.flowIn) annotation(Line(points = {{24, 32}, {26, 32}, {26, -2}, {26, -2}}, color = {0, 127, 255}));
    connect(temperature3.port_b, separator.fedWater) annotation(Line(points = {{4, 18}, {6, 18}, {6, 17}, {7, 17}}, color = {0, 127, 255}));
    connect(temperature3.port_a, OTE2.flowOut) annotation(Line(points = {{-4, 18}, {-4, 18}, {-4, 4}, {0, 4}, {0, -2}, {0, -2}}, color = {0, 127, 255}));
    connect(OTE2.gasIn, SH.gasOut) annotation(Line(points = {{2.2, -14}, {24.2, -14}, {24.2, -14}, {24.2, -14}}, color = {0, 127, 255}));
    connect(SH.gasIn, gasSource.ports[1]) annotation(Line(points = {{36.2, -14}, {64.2, -14}}, color = {0, 127, 255}));
    connect(rampGasFlow.y, gasSource.m_flow_in) annotation(Line(points = {{74.7, -53}, {97.7, -53}, {97.7, -9}, {75.7, -9}}, color = {0, 0, 127}));
    connect(stepGasTemp.y, gasSource.T_in) annotation(Line(points = {{74.7, -31}, {85.2, -31}, {85.2, -29}, {95.7, -29}, {95.7, -10}, {86.2, -10}, {86.2, -12}, {76.7, -12}}, color = {0, 0, 127}));
    connect(separator.drainFeedback, waterSink.m_flow_in) annotation(Line(points = {{19, 13}, {47, 13}, {47, 13}, {77, 13}, {77, 11}, {75, 11}, {75, 9}, {75, 9}}, color = {0, 0, 127}));
    connect(temperature2.port_b, OTE2.flowIn) annotation(Line(points = {{-12, 4}, {-8, 4}, {-8, 0}, {-8, 0}, {-8, 0}, {-8, 0}, {-8, -2}, {-8, -2}}, color = {0, 127, 255}));
    connect(OTE1.flowOut, temperature2.port_a) annotation(Line(points = {{-23.8, -3}, {-23.8, -1.25}, {-23.8, -1.25}, {-23.8, 2.5}, {-23.8, 2.5}, {-23.8, 6}, {-21.8, 6}, {-21.8, 6}, {-20.8, 6}, {-20.8, 4}, {-19.8, 4}}, color = {0, 127, 255}));
    connect(ECO.flowOut, temperature1.port_a) annotation(Line(points = {{-47.8, -3}, {-47.8, -3}, {-47.8, -1}, {-47.8, -1}, {-47.8, -1}, {-47.8, -1}, {-47.8, 7}, {-47.8, 7}, {-47.8, 5}, {-47.8, 5}, {-47.8, 3}, {-47.8, 3}}, color = {0, 127, 255}));
    connect(temperature1.port_b, OTE1.flowIn) annotation(Line(points = {{-40, 4}, {-38, 4}, {-38, 6}, {-36, 6}, {-36, 6}, {-32, 6}, {-32, 2.5}, {-32, 2.5}, {-32, -1.25}, {-32, -1.25}, {-32, -3}}, color = {0, 127, 255}));
    connect(OTE1.gasIn, OTE2.gasOut) annotation(Line(points = {{-21.8, -14}, {-9.8, -14}}, color = {0, 127, 255}));
    connect(ECO.gasIn, OTE1.gasOut) annotation(Line(points = {{-45.8, -14}, {-33.8, -14}}, color = {0, 127, 255}));
    connect(gasSink.ports[1], ECO.gasOut) annotation(Line(points = {{-86, -14}, {-58, -14}}, color = {0, 127, 255}));
    connect(flowSource.ports[1], ECO.flowIn) annotation(Line(points = {{-80, 22}, {-68, 22}, {-68, 24}, {-56, 24}, {-56, 9.5}, {-56, 9.5}, {-56, -3}}, color = {0, 127, 255}));
  initial equation
    for i in 1:numberOfFlueSections_eco loop
      for j in 1:numberOfTubeSections_eco loop
        der(ECO.flowHE.h_flow_n[i, j + 1]) = 0;
  ...
  model testGFHE
    ...
    MyHRSG.GF_HE ECO(redeclare onlyFlowHEBoil_7 flowHE, Din = Dout_eco - 2 * delta_eco, HRSG_type_set = MyHRSG.Choices.HRSG_type.verticalTop, Lpipe = Lpipe, Tinflow = Tinflow_eco, Tingas = Tingas_eco, Toutflow = Toutflow_eco, Toutgas = Toutgas_eco, delta = delta_eco, delta_fin = delta_fin_eco, hfin = hfin_eco, k_gamma_gas = k_gamma_gas_eco, numberOfFlueSections = numberOfFlueSections_eco, numberOfTubeSections = numberOfTubeSections_eco, numberPMCalcSections = numberPMCalcSections_eco, pflow_in = pflow_eco, pflow_out = pflow_eco, pgas = pgas, s1 = s1_eco, s2 = s2_eco, seth_in = hflow_eco_in, seth_out = hflow_eco_out, sfin = sfin_eco, wflow = wflow, wgas = wgas, z1 = z1_eco, z2 = z2_eco, zahod = zahod_eco, setTm = setTm_eco, m_flow_small = m_flow_small) annotation(Placement(visible = true, transformation(origin = {-14, -12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
//end for;
  ...
  model onceThrough_7
    package Medium_F = Modelica.Media.Water.WaterIF97_ph;
    parameter Modelica.SIunits.MassFlowRate wflow = 58 / 3.6 "Пусковой массовый расход воды на входе в сепаратор";
    parameter Modelica.SIunits.MassFlowRate wsteam = 15.2 "Расход пара на выходе из сепаратора";
    parameter Modelica.SIunits.Pressure patm = 1.013e5 "Начальное давление потока вода/пар за клапаном (турбиной)";
    replaceable package Medium_G = MyHRSG.ExhaustGas constrainedby Modelica.Media.Interfaces.PartialMedium;
    parameter Modelica.SIunits.MassFlowRate wgas = 1276.6 / 3.6 "Номинальный (и начальный) массовый расход газов ";
    parameter Modelica.SIunits.Pressure pgas = 3e3 "Начальное давление газов";
    ...
    Modelica.Fluid.Sources.FixedBoundary flowSink(redeclare package Medium = Medium_F, T = Toutflow_ote1, nPorts = 1, p = pflow_eco, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {70, 54}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    ...
    Modelica.Fluid.Sources.MassFlowSource_h waterSink(redeclare package Medium = Medium_F, h = hflow_eco_in, nPorts = 1, use_m_flow_in = true) annotation(Placement(visible = true, transformation(origin = {70, 22}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Ramp rampGasFlow(duration = 20, height = 50, offset = wgas, startTime = 10) annotation(Placement(visible = true, transformation(origin = {70, -66}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.Ramp rampGasTemp(duration = 5, height = 50, offset = Tingas_sh, startTime = 10) annotation(Placement(visible = true, transformation(origin = {70, -34}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    ...
    Modelica.Fluid.Valves.ValveLinear valveLinear1(redeclare package Medium = Medium_F, dp_nominal = 1000, m_flow_nominal = wsteam) annotation(Placement(visible = true, transformation(origin = {23, 67}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
    Modelica.Blocks.Sources.Constant const(k = 1) annotation(Placement(visible = true, transformation(origin = {-2, 80}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(const.y, valveLinear1.opening) annotation(Line(points = {{10, 80}, {24, 80}, {24, 72}, {24, 72}}, color = {0, 0, 127}));
    connect(valveLinear1.port_a, separator.steam) annotation(Line(points = {{18, 68}, {14, 68}, {14, 58}, {20, 58}, {20, 54}, {20, 54}, {20, 54}}, color = {0, 127, 255}));
    connect(valveLinear1.port_b, SH.flowIn) annotation(Line(points = {{28, 68}, {32, 68}, {32, 38}, {30, 38}, {30, 24}, {30, 24}}, color = {0, 127, 255}));
    connect(SH.flowOut, flowSink.ports[1]) annotation(Line(points = {{38, 24}, {38, 24}, {38, 54}, {60, 54}, {60, 54}}, color = {0, 127, 255}));
...
//Граничные условия
  ...
  model onceThrough_8
    ...
    //Исходные данные по разбиению пароперегревателя (SH)
    parameter Integer numberOfTubeSections_sh = 2 "Число участков разбиения трубы" annotation(Dialog(group = "Конструктивные характеристики"));
    parameter Integer numberPMCalcSections_sh = 2 "Число участков разбиения трубы входящих в один участок расчета процессов массообмена" annotation(Dialog(group = "Конструктивные характеристики"));
    parameter Integer numberOfFlueSections_sh = z2_sh "Число участков разбиения газохода" annotation(Dialog(group = "Конструктивные характеристики"));
    //Исходные данные вода/пар для пароперегревателя
    parameter Modelica.SIunits.Pressure pflow_sh = 10.01e5 "Начальное давление потока вода/пар перед SH";
    parameter Modelica.SIunits.Temperature Tinflow_sh = 191 + 273.15 "Начальная входная температура потока воды/пар";
    ...
    GF_HE SH(redeclare onlyFlowHEBoilLite flowHE, redeclare package Medium_G = Medium_G, wgas = wgas, pgas = pgas, Tingas = Tingas_sh, Toutgas = Tingas_sh, k_gamma_gas = k_gamma_gas_sh, redeclare package Medium_F = Medium_F, wflow = wsteam, pflow_in = pflow_sh, pflow_out = pflow_sh, Tinflow = Tinflow_sh, Toutflow = Tinflow_sh, numberOfTubeSections = numberOfTubeSections_sh, numberPMCalcSections = numberPMCalcSections_sh, numberOfFlueSections = numberOfFlueSections_sh, Din = Dout_sh - 2 * delta_sh, delta = delta_sh, s1 = s1_sh, s2 = s2_sh, zahod = zahod_sh, z1 = z1_sh, z2 = z2_sh, Lpipe = Lpipe, delta_fin = delta_fin_sh, hfin = hfin_sh, sfin = sfin_sh, seth_in = hflow_sh_in, seth_out = hflow_sh_out) annotation(Placement(visible = true, transformation(origin = {34, 12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  equation
    connect(separator.steam, SH.flowIn) annotation(Line(points = {{20, 54}, {20, 54}, {20, 58}, {30, 58}, {30, 24}, {30, 24}}, color = {0, 127, 255}));
    connect(SH.flowOut, flowSink.ports[1]) annotation(Line(points = {{38, 24}, {38, 24}, {38, 54}, {60, 54}, {60, 54}}, color = {0, 127, 255}));
//separator.steam.p = pflow_eco;
//SH.flowIn.m_flow = wsteam;
//SH.flowIn.h_outflow = hflow_sh_in;
...
/*for i in 1:1 loop
  ...
  //for j in 2:integer(numberOfTubeSections_sh / numberPMCalcSections_sh) loop
  //der(SH.flowHE.p_flow_v[i, j]) = 0;
  //end for;
    end for;*/
    for i in 1:numberOfFlueSections_sh loop
...
//end for;
    ...
    model onceThrough_13
      ...
      Modelica.Fluid.Sources.MassFlowSource_T gasSource(redeclare package Medium = Medium_G, nPorts = 1, use_T_in = true, use_m_flow_in = true) annotation(Placement(visible = true, transformation(origin = {70, -6}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
      Modelica.Fluid.Sources.FixedBoundary gasSink(redeclare package Medium = Medium_G, T = Toutgas_eco, nPorts = 1, p = pgas, use_T = true, use_p = true) annotation(Placement(visible = true, transformation(origin = {-90, -6}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      MyHRSG.GF_HE OTE1(redeclare onlyFlowHEBoil_7 flowHE, redeclare package Medium_G = Medium_G, HRSG_type_set = MyHRSG.Choices.HRSG_type.verticalTop, wgas = wgas, pgas = pgas, Tingas = Tingas_ote1, Toutgas = Tingas_ote1, k_gamma_gas = k_gamma_gas_ote1, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_ote1, pflow_out = pflow_ote1, Tinflow = Tinflow_ote1, Toutflow = Tinflow_ote1, numberOfTubeSections = numberOfTubeSections_ote1, numberPMCalcSections = numberPMCalcSections_ote1, numberOfFlueSections = numberOfFlueSections_ote1, Din = Din_ote1, delta = delta_ote1, s1 = s1_ote1, s2 = s2_ote1, zahod = zahod_ote1, z1 = z1_ote1, z2 = z2_ote1, Lpipe = Lpipe, delta_fin = delta_fin_ote1, hfin = hfin_ote1, sfin = sfin_ote1, seth_in = hflow_ote1_in, seth_out = hflow_ote1_out, setTm = setTm_ote1) annotation(Placement(visible = true, transformation(origin = {-22, 12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      MyHRSG.GF_HE OTE2(redeclare onlyFlowHEBoil_7 flowHE, redeclare package Medium_G = Medium_G, HRSG_type_set = MyHRSG.Choices.HRSG_type.verticalTop, wgas = wgas, pgas = pgas, Tingas = Tingas_ote2, Toutgas = Tingas_ote2, k_gamma_gas = k_gamma_gas_ote2, redeclare package Medium_F = Medium_F, wflow = wflow, pflow_in = pflow_ote2, pflow_out = pflow_ote2, Tinflow = Tinflow_ote2, Toutflow = Tinflow_ote2, numberOfTubeSections = numberOfTubeSections_ote2, numberPMCalcSections = numberPMCalcSections_ote2, numberOfFlueSections = numberOfFlueSections_ote2, Din = Din_ote2, delta = delta_ote2, s1 = s1_ote2, s2 = s2_ote2, zahod = zahod_ote2, z1 = z1_ote2, z2 = z2_ote2, Lpipe = Lpipe, delta_fin = delta_fin_ote2, hfin = hfin_ote2, sfin = sfin_ote2, seth_in = hflow_ote2_in, seth_out = hflow_ote2_out, setTm = setTm_ote2) annotation(Placement(visible = true, transformation(origin = {2, 12}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Modelica.Fluid.Sensors.TemperatureTwoPort temperature1(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {-38, 30}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
      Modelica.Fluid.Sensors.TemperatureTwoPort temperature2(redeclare package Medium = Medium_F) annotation(Placement(visible = true, transformation(origin = {-10, 30}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
      Modelica.Blocks.Sources.Ramp rampGasFlow(duration = 600, height = 1000, offset = wgas, startTime = 10) annotation(Placement(visible = true, transformation(origin = {70, -66}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Modelica.Blocks.Sources.Ramp rampGasTemp(duration = 40, height = 200, offset = Tingas_sh, startTime = 10) annotation(Placement(visible = true, transformation(origin = {70, -34}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      ...
      Modelica.Fluid.Valves.ValveLinear CV2(redeclare package Medium = Medium_F, dp_nominal = 10.01e5 - system.p_ambient, m_flow_nominal = wsteam) annotation(Placement(visible = true, transformation(origin = {47, 57}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
      Modelica.Blocks.Sources.Constant constCV2 annotation(Placement(visible = true, transformation(origin = {36, 86}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      MyHRSG.Separator2 separator21 annotation(Placement(visible = true, transformation(origin = {14, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
...
//Граничные условия для высотной отметки входного коллектора
  ...
  model Separator2
    ...
    //***Исходные данные
    replaceable package Medium = Modelica.Media.Water.WaterIF97_ph constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium;
    parameter Medium.AbsolutePressure ps_start "Стартовое давление насыщения в сепараторе";
    parameter Medium.MassFlowRate Dsteam_start "Стартовый расход пара из сепаратора";
  ...
  equation
    ...
    h_dew = Medium.dewEnthalpy(sat);
    h_bubble = Medium.bubbleEnthalpy(sat);
    Dsteam = if inStream(fedWater.h_outflow) > h_dew then D_fw elseif inStream(fedWater.h_outflow) < h_bubble then 0 else (D_fw * inStream(fedWater.h_outflow) - (D_fw - Dsteam) * h_bubble) / h_dew;
//Питательная вода
...
//Выход насыщенного пара
    ps = steam.p;
    steam.h_outflow = if inStream(fedWater.h_outflow) > h_dew then inStream(fedWater.h_outflow) else h_dew;
    steam.m_flow = min(-Dsteam, -m_flow_small);
    annotation(uses(Modelica(version = "3.2.1")));
  end Separator2;

  model onlyFlowHEBoil_3 "Изменены ф-лы сопротивления и С1, С2 вместо функции HTtoMT() везде суммы"
    function positiveMax
      extends Modelica.Icons.Function;
      input Real x;
      output Real y;
...
//Осреднение по конечному объему
//p_flow_v[i, j] = (p_flow_n[i, j + 1] + p_flow_n[i, j]) / 2;
//p_flow_v[i, j] = p_flow_n[i, j] - 0.5 * max(dp_fric[i, j] + dp_piez[i, j], 0);
        p_flow_v[i, j] = p_flow_n[i, j];
        D_flow_v[i, j] = (D_flow_n[i, j + 1] + D_flow_n[i, j]) / 2;
//Основное уравнение гидравлики
        wrhop[i, j] = HTtoMT(w_flow_v[i, :], numberPMCalcSections, j) * HTtoMT(rho_flow_v[i, :], numberPMCalcSections, j) * p_flow_v[i, j] * 10 ^ (-5) "Произведение wrhop для расчета phi [кг/(м2*с)*кгс/см2]";
        Xi_flow[i, j] = lambda_tr(Din, ke, HTtoMT(Re_flow[i, :], numberPMCalcSections, j)) * Lpipe / Din / numberOfTubeSections * numberPMCalcSections;
        phi[i, j] = phi_heatedPipe(wrhop[i, j], p_flow_v[i, j] / 100000, HTtoMT(x_v[i, :], numberPMCalcSections, j)) "Расчет коэффициента phi";
        dp_fric[i, j] = if HTtoMT(x_v[i, :], numberPMCalcSections, j) < 1 and HTtoMT(x_v[i, :], numberPMCalcSections, j) > 0 then HTtoMT(w_flow_v[i, :], numberPMCalcSections, j) ^ 2 * Xi_flow[i, j] * bubble_rho_flow_v[i, j] / 2 / Modelica.Constants.g_n * (1 + HTtoMT(x_v[i, :], numberPMCalcSections, j) * phi[i, j] * (HTtoMT(bubble_rho_flow_v[i, :], numberPMCalcSections, j) / HTtoMT(dew_rho_flow_v[i, :], numberPMCalcSections, j) - 1)) else HTtoMT(w_flow_v[i, :], numberPMCalcSections, j) ^ 2 * Xi_flow[i, j] * HTtoMT(rho_flow_v[i, :], numberPMCalcSections, j) / 2 / Modelica.Constants.g_n "Потеря давления от трения";
        if HRSG_type == MyHRSG.Choices.HRSG_type.horizontalBottom then
          H_flow[i, j + 1] = H_flow[i, j] - hod[i] * deltaHpipe "Расчет высотных отметок для горизонтального КУ";
        elseif HRSG_type == MyHRSG.Choices.HRSG_type.horizontalTop then
          H_flow[i, j + 1] = H_flow[i, j] + hod[i] * deltaHpipe "Расчет высотных отметок для горизонтального КУ";
...
//Граничные условия для высотной отметки входного коллектора
    ...
    waterIn.h_outflow = sum(array(positiveMax(D_flow_n[i, 1]) * h_flow_n[i, 1] for i in 1:zahod)) / sum(array(positiveMax(D_flow_n[i, 1]) for i in 1:zahod));
    annotation(Documentation(info = "<HTML>Модель теплообменника с heatPort. Моделируется несколько ходов. Кипение. Модель воды - Modelica.Media.Water.WaterIF97_ph. Первый заход труб номеруется с 1, второй также с 1. Т.е. во всех заходах поток с одним знаком, и разность давлений с одним знаком (другое описание гибов).</html>"), Diagram(graphics), experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02), Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2}), graphics = {Rectangle(lineColor = {0, 0, 255}, fillColor = {230, 230, 230}, fillPattern = FillPattern.Solid, extent = {{-100, 100}, {100, -100}}), Line(points = {{0, -80}, {0, -40}, {40, -20}, {-40, 20}, {0, 40}, {0, 80}}, color = {0, 0, 255}, thickness = 0.5), Text(origin = {-2, 52}, lineColor = {85, 170, 255}, extent = {{-100, -115}, {100, -145}}, textString = "%name")}));
  end onlyFlowHEBoil_3;

  model onlyFlowHEBoil_4
    function positiveMax
      extends Modelica.Icons.Function;
      input Real x;
      output Real y;
    ...
    function z2toZahod "Функция перехода с номера трубы по ходу потока на заход на котором труба находится"
      input Real x[:] "Массив элементов по участком тубы разбитых для расчета массообмена";
      input Integer zahod "Заходность";
      input Integer j "Текущий номер трубы по ходу потока";
      output Real y;
    algorithm
      y := x[j - (integer((j + zahod - 1) / zahod) - 1) * zahod];
    end z2toZahod;

    function razbivka
      input Real firstValue[zahod] "первое значение";
      input Real endValue[zahod] "последнее значение";
      input Integer zahod "число заходов";
      input Integer numberOfFlueSections;
      input Integer numberOfTubeSectionsForMT;
      input Integer i;
      input Integer j;
      output Real x_out;
    protected
      Real delta[zahod] "разность между значениями входящими в вектор";
      Real x[numberOfFlueSections, numberOfTubeSectionsForMT + 1];
    algorithm
      for k in 1:zahod loop
        delta[k] := (endValue[k] - firstValue[k]) / numberOfFlueSections * zahod / numberOfTubeSectionsForMT;
        for j in 1:numberOfFlueSections / zahod loop
          for i in 1:numberOfTubeSectionsForMT + 1 loop
            if j == 1 then
              x[k + zahod * (j - 1), i] := firstValue[k] + (i - 1) * delta[k];
            else
              x[k + zahod * (j - 1), i] := x[k + zahod * (j - 2), numberOfTubeSectionsForMT + 1] + (i - 1) * delta[k];
            end if;
          end for;
        end for;
      end for;
    ...
    Medium_F.MassFlowRate D_flow_v[numberOfFlueSections, numberOfTubeSectionsForMT](start = D_startFlow_v) "Массовый расход потока вода/пар по участкам ряда труб";
    Medium_F.MassFlowRate D_flow_n[numberOfFlueSections, numberOfTubeSectionsForMT + 1](start = D_startFlow_n) "Массовый расход потока вода/пар по участкам ряда труб";
    Medium_F.MassFlowRate deltaD_flow_n[zahod] "Изменение массового расхода во всей поверхности";
    Modelica.SIunits.CoefficientOfHeatTransfer alfa_flow[numberOfFlueSections, numberOfTubeSections] "Коэффициент теплопередачи со стороны потока вода/пар";
    Medium_F.ThermalConductivity k_flow[numberOfFlueSections, numberOfTubeSections] "Коэффициент теплопроводности для потока вода/пар";
    Medium_F.DynamicViscosity mu_flow[numberOfFlueSections, numberOfTubeSections] "Динамическая вязкость для потока вода/пар";
    Modelica.SIunits.HeatFlowRate Q_flow[numberOfFlueSections, numberOfTubeSections] "тепло переданное стенке трубы";
    ...
    Real Re_flow[numberOfFlueSections, numberOfTubeSections] "Число Рейнольдса";
    Modelica.SIunits.Temperature t_m[numberOfFlueSections, numberOfTubeSections](start = t_startM) "Температура металла на участках трубопровода";
    Real C1[zahod] "Показатель в числителе уравнения сплошности";
    Real C2[zahod] "Показатель в знаменателе уравнения сплошности";
    Real hod[numberOfFlueSections] "Четность или не четность текущего хода теплообменника (минус 1 - нечетный, плюс 1 - четный)";
    Modelica.SIunits.Length H_flow[numberOfFlueSections, numberOfTubeSectionsForMT + 1] "Высотная отметка каждого узла";
    Modelica.SIunits.Velocity w_flow_v[numberOfFlueSections, numberOfTubeSections] "Скорость потока вода/пар в конечных объемах";
    Modelica.SIunits.Velocity w_flow_n[numberOfFlueSections, numberOfTubeSectionsForMT + 1] "Скорость потока вода/пар в узловых точках";
...
//Уравнения для расчета процессов массообмена
      for j in 1:numberOfTubeSectionsForMT loop
        ...
        bubble_rho_flow_v[i, j] = Medium_F2.bubbleDensity(sat_v[i, j]);
//Осреднение по конечному объему
        p_flow_v[i, j] = p_flow_n[i, j + 1] - (dp_fric[i, j] + dp_piez[i, j]) / 2;
        D_flow_v[i, j] = (D_flow_n[i, j + 1] + D_flow_n[i, j]) / 2;
//Основное уравнение гидравлики
        wrhop[i, j] = HTtoMT(w_flow_v[i, :], numberPMCalcSections, j) * HTtoMT(rho_flow_v[i, :], numberPMCalcSections, j) * p_flow_v[i, j] * 10 ^ (-5) "Произведение wrhop для расчета phi [кг/(м2*с)*кгс/см2]";
        Xi_flow[i, j] = lambda_tr(Din, ke, HTtoMT(Re_flow[i, :], numberPMCalcSections, j)) * Lpipe / Din / numberOfTubeSections * numberPMCalcSections;
        ...
        dp_piez[i, j] = (rho_flow_n[i, j + 1] * H_flow[i, j + 1] - rho_flow_n[i, j] * H_flow[i, j]) * Modelica.Constants.g_n "Расчет перепада давления из-за изменения пьезометрической высоты";
        p_flow_n[i, j] - p_flow_n[i, j + 1] = max(dp_fric[i, j] + dp_piez[i, j], 0) "Формула 2-1 из книги Рудомино, Ремжин";
        D_flow_n[i, j + 1] = sum(waterIn.m_flow / zahod - z2toZahod(deltaD_flow_n[:], zahod, i) * m for m in 1:j + (integer((i + zahod - 1) / zahod) - 1 - 1) * numberOfTubeSectionsForMT);
      end for;
    end for;
//integer((j + numberPMCalcSections - 1) / numberPMCalcSections)
/*for i in 1:numberOfFlueSections - zahod loop
      for j in 2:numberOfTubeSectionsForMT + 1 loop
        //D_flow_n[i, j] = razbivka(D_flow_n[1 : zahod, 1], D_flow_n[numberOfFlueSections - zahod + 1 : numberOfFlueSections, numberOfTubeSectionsForMT + 1], zahod, numberOfFlueSections, numberOfTubeSectionsForMT, i, j);
        D_flow_n[i, j] = D_flow_n[1, 1] + 0.5 * z2toZahod(deltaD_flow_n[:], zahod, i); 
      end for;
    end for;
    
    for i in numberOfFlueSections - zahod + 1:numberOfFlueSections loop
      for j in 2:numberOfTubeSectionsForMT loop
        //D_flow_n[i, j] = razbivka(D_flow_n[1 : zahod, 1], D_flow_n[numberOfFlueSections - zahod + 1 : numberOfFlueSections, numberOfTubeSectionsForMT + 1], zahod, numberOfFlueSections, numberOfTubeSectionsForMT, i, j);
        D_flow_n[i, j] = D_flow_n[1, 1] + 0.5 * z2toZahod(deltaD_flow_n[:], zahod, i);
      end for;  
    end for;*/
    for i in 1:zahod loop
      deltaD_flow_n[i] = C1[i] + C2[i] "Уравнение сплошности (формула 3-6 диссертации Рубашкина)";
      C1[i] = sum(deltaVFlow * drdh_flow[i + zahod * (m - 1), n] * der_h_flow_v[i + zahod * (m - 1), n] for m in 1:numberOfFlueSections / zahod, n in 1:numberOfTubeSections);
      C2[i] = sum(deltaVFlow * drdp_flow[i + zahod * (m - 1), n] for m in 1:numberOfFlueSections / zahod, n in 1:numberOfTubeSections) * der(p_flow_v[i + integer(numberOfFlueSections / 2) - integer(zahod / 2), integer(numberOfTubeSectionsForMT / 2) + 1]);
//D_flow_n[i, 1] - D_flow_n[numberOfFlueSections - (zahod - i), numberOfTubeSectionsForMT + 1] = deltaD_flow_n[i];
    end for;
    for i in 1:numberOfFlueSections - zahod loop
//Описание гибов
      D_flow_n[i, numberOfTubeSectionsForMT + 1] = D_flow_n[i + zahod, 1];
...
//Граничные условия для высотной отметки входного коллектора
    ...
    waterIn.h_outflow = sum(array(positiveMax(D_flow_n[i, 1]) * h_flow_n[i, 1] for i in 1:zahod)) / sum(array(positiveMax(D_flow_n[i, 1]) for i in 1:zahod));
    annotation(Documentation(info = "<HTML>Модель теплообменника с heatPort. Моделируется несколько ходов. Кипение. Модель воды - Modelica.Media.Water.WaterIF97_ph. Первый заход труб номеруется с 1, второй также с 1. Т.е. во всех заходах поток с одним знаком, и разность давлений с одним знаком (другое описание гибов).</html>"), Diagram(graphics), experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02), Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2}), graphics = {Rectangle(lineColor = {0, 0, 255}, fillColor = {230, 230, 230}, fillPattern = FillPattern.Solid, extent = {{-100, 100}, {100, -100}}), Line(points = {{0, -80}, {0, -40}, {40, -20}, {-40, 20}, {0, 40}, {0, 80}}, color = {0, 0, 255}, thickness = 0.5), Text(origin = {-2, 52}, lineColor = {85, 170, 255}, extent = {{-100, -115}, {100, -145}}, textString = "%name")}));
  end onlyFlowHEBoil_4;

  model onlyFlowHEBoil_5
    function positiveMax
      extends Modelica.Icons.Function;
      input Real x;
      output Real y;
    ...
    parameter Medium_F.SpecificEnthalpy h_startFlow_n[numberOfFlueSections, numberOfTubeSections + 1] = fill(seth_in, numberOfFlueSections, numberOfTubeSections + 1) "Начальный вектор энальпии потока газов" annotation(Dialog(tab = "Инициализация"));
    parameter Medium_F.SpecificEnthalpy h_startFlow_v[numberOfFlueSections, numberOfTubeSections] = fill(seth_in, numberOfFlueSections, numberOfTubeSections) "Начальный вектор энальпии потока газов" annotation(Dialog(tab = "Инициализация"));
    parameter Medium_F.AbsolutePressure p_startFlow_v[zahod] = fill(setp_flow_in, zahod) "Начальный вектор давлений потока вода/пар" annotation(Dialog(tab = "Инициализация"));
    parameter Medium_F.AbsolutePressure p_startFlow_n[numberOfFlueSections, numberOfTubeSectionsForMT + 1] = fill(setp_flow_in, numberOfFlueSections, numberOfTubeSectionsForMT + 1) "Начальный вектор давлений потока вода/пар" annotation(Dialog(tab = "Инициализация"));
    parameter Medium_F.MassFlowRate D_startFlow_v[numberOfFlueSections, numberOfTubeSectionsForMT] = fill(setD_flow / zahod, numberOfFlueSections, numberOfTubeSectionsForMT) "Начальный вектор массового расхода потока вода/пар по конечным объемам" annotation(Dialog(tab = "Инициализация"));
    parameter Medium_F.MassFlowRate D_startFlow_n[numberOfFlueSections, numberOfTubeSectionsForMT + 1] = fill(setD_flow / zahod, numberOfFlueSections, numberOfTubeSectionsForMT + 1) "Начальный вектор массового расхода потока вода/пар по узловым точкам" annotation(Dialog(tab = "Инициализация"));
    //Металл
    ...
    Medium_F2.ThermodynamicState stateFlowTwoPhase[numberOfFlueSections, numberOfTubeSections] "Термодинамическое состояние потока вода/пар на участках трубопровода";
    Medium_F.Temperature t_flow[numberOfFlueSections, numberOfTubeSections] "Температура потока вода/пар по участкам трубы";
    Medium_F.AbsolutePressure p_flow_v[zahod](start = p_startFlow_v) "Давление потока вода/пар по участкам трубы в конечных объемах";
    Medium_F.AbsolutePressure p_flow_n[numberOfFlueSections, numberOfTubeSectionsForMT + 1](start = p_startFlow_n) "Давление потока вода/пар по участкам трубы в узловых точках";
    Medium_F.SpecificEnthalpy h_flow_v[numberOfFlueSections, numberOfTubeSections](start = h_startFlow_v) "Энтальпия потока вода/пар по участкам трубы в конечных объемах";
    Medium_F.SpecificEnthalpy h_flow_n[numberOfFlueSections, numberOfTubeSections + 1](start = h_startFlow_n) "Энтальпия потока вода/пар по участкам трубы в узловых точках";
    Real der_h_flow_v[numberOfFlueSections, numberOfTubeSections] "Производняа энтальпии потока вода/пар";
...
//Уравнения состояния
        stateFlow[i, j] = Medium_F.setState_ph(z2toZahod(p_flow_v[:], zahod, i), h_flow_v[i, j]);
        t_flow[i, j] = Medium_F.temperature(stateFlow[i, j]);
        rho_flow_v[i, j] = Medium_F.density(stateFlow[i, j]);
        drdp_flow[i, j] = max(min(Medium_F.density_derp_h(stateFlow[i, j]), 2.0394e-4), 3.0591e-6);
        drdh_flow[i, j] = max(Medium_F.density_derh_p(stateFlow[i, j]), -0.0191);
//Коэффициент теплоотдачи
        ...
        assert(t_m[i, j] < t_flow[i, j], "Temperatura metalla nije temperaturi potoka", level = AssertionLevel.warning);
//Про две фазы
        stateFlowTwoPhase[i, j] = Medium_F2.setState_ph(z2toZahod(p_flow_v[:], zahod, i), h_flow_v[i, j]);
        x_v[i, j] = Medium_F2.vapourQuality(stateFlowTwoPhase[i, j]);
      end for;
//Уравнения для расчета процессов массообмена
      for j in 1:numberOfTubeSectionsForMT loop
        sat_v[i, j] = Medium_F2.setSat_p(z2toZahod(p_flow_v[:], zahod, i));
        dew_rho_flow_v[i, j] = Medium_F2.dewDensity(sat_v[i, j]);
        bubble_rho_flow_v[i, j] = Medium_F2.bubbleDensity(sat_v[i, j]);
//Осреднение по конечному объему
        D_flow_v[i, j] = (D_flow_n[i, j + 1] + D_flow_n[i, j]) / 2;
//Основное уравнение гидравлики
        wrhop[i, j] = HTtoMT(w_flow_v[i, :], numberPMCalcSections, j) * HTtoMT(rho_flow_v[i, :], numberPMCalcSections, j) * z2toZahod(p_flow_v[:], zahod, i) * 10 ^ (-5) "Произведение wrhop для расчета phi [кг/(м2*с)*кгс/см2]";
        Xi_flow[i, j] = lambda_tr(Din, ke, HTtoMT(Re_flow[i, :], numberPMCalcSections, j)) * Lpipe / Din / numberOfTubeSections * numberPMCalcSections;
        phi[i, j] = phi_heatedPipe(wrhop[i, j], z2toZahod(p_flow_v[:], zahod, i) / 100000, HTtoMT(x_v[i, :], numberPMCalcSections, j)) "Расчет коэффициента phi";
        dp_fric[i, j] = sum(w_flow_v[i, k] ^ 2 * Xi_flow[i, j] * max(bubble_rho_flow_v[i, j], rho_flow_v[i, k]) / 2 / Modelica.Constants.g_n * (1 + x_v[i, k] * phi[i, j] * (bubble_rho_flow_v[i, j] / dew_rho_flow_v[i, j] - 1)) for k in 1 + (j - 1) * numberPMCalcSections:j * numberPMCalcSections) "Потеря давления от трения";
        if HRSG_type == MyHRSG.Choices.HRSG_type.horizontalBottom then
          H_flow[i, j + 1] = H_flow[i, j] - hod[i] * deltaHpipe "Расчет высотных отметок для горизонтального КУ";
        elseif HRSG_type == MyHRSG.Choices.HRSG_type.horizontalTop then
...
//p_flow_n[i, j] - p_flow_n[i, j + 1] = dp_fric[i, j] + dp_kin[i, j] + dp_piez[i, j] "Формула 2-1 из книги Рудомино, Ремжин";
        ...
        C1[i, j] = sum(deltaVFlow * drdh_flow[i, k] * der_h_flow_v[i, k] for k in 1 + (j - 1) * numberPMCalcSections:j * numberPMCalcSections);
//Возможно не верно указан объем (объем для расчета теплообмена а надо для массообмена)
        C2[i, j] = sum(deltaVFlow * drdp_flow[i, k] for k in 1 + (j - 1) * numberPMCalcSections:j * numberPMCalcSections) * der(z2toZahod(p_flow_v[:], zahod, i));
      end for;
    end for;
    for i in 1:zahod loop
//p_flow_v[i] = p_flow_n[numberOfFlueSections - (zahod - i), numberOfTubeSectionsForMT + 1] + 0.5 * sum(max(dp_fric[i + zahod * (m - 1), n] + dp_piez[i + zahod * (m - 1), n], 0) for m in 1:numberOfFlueSections/zahod, n in 1:numberOfTubeSectionsForMT);
      p_flow_v[i] = 0.5 * (p_flow_n[zahod, 1] + p_flow_n[numberOfFlueSections - (zahod - i), numberOfTubeSectionsForMT + 1]);
    end for;
    for i in 1:numberOfFlueSections - zahod loop
//Описание гибов
      D_flow_n[i, numberOfTubeSectionsForMT + 1] = D_flow_n[i + zahod, 1];
...
//Граничные условия для высотной отметки входного коллектора
    ...
    waterIn.h_outflow = sum(array(positiveMax(D_flow_n[i, 1]) * h_flow_n[i, 1] for i in 1:zahod)) / sum(array(positiveMax(D_flow_n[i, 1]) for i in 1:zahod));
    annotation(Documentation(info = "<HTML>Модель теплообменника с heatPort. Моделируется несколько ходов. Кипение. Модель воды - Modelica.Media.Water.WaterIF97_ph. Первый заход труб номеруется с 1, второй также с 1. Т.е. во всех заходах поток с одним знаком, и разность давлений с одним знаком (другое описание гибов).</html>"), Diagram(graphics), experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02), Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2}), graphics = {Rectangle(lineColor = {0, 0, 255}, fillColor = {230, 230, 230}, fillPattern = FillPattern.Solid, extent = {{-100, 100}, {100, -100}}), Line(points = {{0, -80}, {0, -40}, {40, -20}, {-40, 20}, {0, 40}, {0, 80}}, color = {0, 0, 255}, thickness = 0.5), Text(origin = {-2, 52}, lineColor = {85, 170, 255}, extent = {{-100, -115}, {100, -145}}, textString = "%name")}));
  end onlyFlowHEBoil_5;

  model onlyFlowHEBoil_6
    function positiveMax
      extends Modelica.Icons.Function;
      input Real x;
      output Real y;
    ...
    parameter Medium_F.SpecificEnthalpy h_startFlow_v[numberOfFlueSections, numberOfTubeSections] = fill(seth_in, numberOfFlueSections, numberOfTubeSections) "Начальный вектор энальпии потока газов" annotation(Dialog(tab = "Инициализация"));
    parameter Medium_F.AbsolutePressure p_startFlow_v[zahod] = fill(setp_flow_in, zahod) "Начальный вектор давлений потока вода/пар" annotation(Dialog(tab = "Инициализация"));
    parameter Medium_F.AbsolutePressure p_startFlow_n[zahod, 2] = fill(setp_flow_in, zahod, 2) "Начальный вектор давлений потока вода/пар" annotation(Dialog(tab = "Инициализация"));
    ...
    Medium_F.AbsolutePressure p_flow_n[zahod, 2](start = p_startFlow_n) "Давление потока вода/пар по участкам трубы в узловых точках";
    Medium_F.SpecificEnthalpy h_flow_v[numberOfFlueSections, numberOfTubeSections](start = h_startFlow_v) "Энтальпия потока вода/пар по участкам трубы в конечных объемах";
    Medium_F.SpecificEnthalpy h_flow_n[numberOfFlueSections, numberOfTubeSections + 1](start = h_startFlow_n) "Энтальпия потока вода/пар по участкам трубы в узловых точках";
    Real der_h_flow_v[numberOfFlueSections, numberOfTubeSections] "Производняа энтальпии потока вода/пар";
    Medium_F.Density rho_flow_v[numberOfFlueSections, numberOfTubeSections] "Плотность потока по участкам трубы в конечных объемах";
    Medium_F.Density rho_flow_v_av[zahod] "Осредненная по заходу плотность потока по участкам трубы в конечных объемах";
    Medium_F.Density rho_flow_n[2] "Плотность потока по участкам трубы в узловых точках";
    Modelica.SIunits.DerDensityByEnthalpy drdh_flow[numberOfFlueSections, numberOfTubeSections] "Производная плотности потока по энтальпии на участках ряда труб";
    Modelica.SIunits.DerDensityByPressure drdp_flow[numberOfFlueSections, numberOfTubeSections] "Производная плотности потока по давлению на участках ряда труб";
    ...
    Real Pr_flow[numberOfFlueSections, numberOfTubeSections] "Число Прандтля для потока вода/пар";
    Real Re_flow[numberOfFlueSections, numberOfTubeSections] "Число Рейнольдса";
    Real Re_flow_av[zahod] "Число Рейнольдса осредненное по заходу";
    Modelica.SIunits.Temperature t_m[numberOfFlueSections, numberOfTubeSections](start = t_startM) "Температура металла на участках трубопровода";
    ...
    Modelica.SIunits.Velocity w_flow_v[numberOfFlueSections, numberOfTubeSections] "Скорость потока вода/пар в конечных объемах";
    Modelica.SIunits.Velocity w_flow_v_av[zahod] "Средняя по заходу скорость потока вода/пар в конечных объемах";
    Real dp_fric[zahod] "Потеря давления из-за сил трения";
    Real dp_piez "Перепад давления из-за изменения пьезометрической высоты";
    Medium_F2.SaturationProperties sat_v[zahod] "State vector to compute saturation properties внутри конечного объема";
    Real wrhop[zahod] "Произведение массовой скорости на давление внутри конечного объема для поправочного коэффициента phi";
    Real phi[zahod] "Коэффициент для расчета гидравлического сопротивления двухфазного потока";
    Real Xi_flow[zahod] "Коэффициент гидравлического сопротивления участка трубы";
    Real x_v[numberOfFlueSections, numberOfTubeSections] "Степень сухости";
    Real x_v_av[zahod] "Степень сухости осредненная по заходу";
    Medium_F.Density dew_rho_flow_v[zahod] "Плотность пара на линии насыщения по участкам трубы в конечных объемах";
    Medium_F.Density bubble_rho_flow_v[zahod] "Плотность  на воды линии насыщения по участкам трубы в конечных объемах";
    //**
    //Интерфейс
    //**
    Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_b heat[numberOfFlueSections, numberOfTubeSections] annotation(Placement(visible = false, transformation(origin = {16, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {120, -100}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
...
//Уравнения состояния
        stateFlow[i, j] = Medium_F.setState_ph(p_flow_v[i - (integer((i + zahod - 1) / zahod) - 1) * zahod], h_flow_v[i, j]);
        t_flow[i, j] = Medium_F.temperature(stateFlow[i, j]);
        rho_flow_v[i, j] = Medium_F.density(stateFlow[i, j]);
        drdp_flow[i, j] = max(min(Medium_F.density_derp_h(stateFlow[i, j]), 2.0394e-4), 3.0591e-6);
        drdh_flow[i, j] = max(Medium_F.density_derh_p(stateFlow[i, j]), -0.0191);
...
//Про две фазы
        stateFlowTwoPhase[i, j] = Medium_F2.setState_ph(p_flow_v[i - (integer((i + zahod - 1) / zahod) - 1) * zahod], h_flow_v[i, j]);
        x_v[i, j] = Medium_F2.vapourQuality(stateFlowTwoPhase[i, j]);
        D_flow_v[i, j] = (D_flow_n[i, j + 1] + D_flow_n[i, j]) / 2;
        D_flow_n[i, j] - D_flow_n[i, j + 1] = C1[i, j] + C2[i, j] "Уравнение сплошности (формула 3-6 диссертации Рубашкина)";
        C1[i, j] = deltaVFlow * drdh_flow[i, j] * der_h_flow_v[i, j];
//Возможно не верно указан объем (объем для расчета теплообмена а надо для массообмена)
        C2[i, j] = deltaVFlow * drdp_flow[i, j] * der_p_flow_v[i - (integer((i + zahod - 1) / zahod) - 1) * zahod];
      end for;
    end for;
    for i in 1:zahod loop
//Уравнения для расчета процессов массообмена
      sat_v[i] = Medium_F2.setSat_p(p_flow_v[i]);
      dew_rho_flow_v[i] = Medium_F2.dewDensity(sat_v[i]);
      bubble_rho_flow_v[i] = Medium_F2.bubbleDensity(sat_v[i]);
//Осреднение по конечному объему
      p_flow_v[i] = (p_flow_n[i, 1] + p_flow_n[i, 2]) / 2;
      der_p_flow_v[i] = der(p_flow_v[i]);
//Основное уравнение гидравлики
      w_flow_v_av[i] = sum(w_flow_v[i + zahod * (k - 1), j] for k in 1:numberOfFlueSections / zahod, j in 1:numberOfTubeSections) / (numberOfFlueSections / zahod) / numberOfTubeSections;
      rho_flow_v_av[i] = sum(w_flow_v[i + zahod * (k - 1), j] for k in 1:numberOfFlueSections / zahod, j in 1:numberOfTubeSections) / (numberOfFlueSections / zahod) / numberOfTubeSections;
      Re_flow_av[i] = sum(Re_flow[i + zahod * (k - 1), j] for k in 1:numberOfFlueSections / zahod, j in 1:numberOfTubeSections) / (numberOfFlueSections / zahod) / numberOfTubeSections;
      wrhop[i] = w_flow_v_av[i] * rho_flow_v_av[i] * p_flow_v[i] * 10 ^ (-5) "Произведение wrhop для расчета phi [кг/(м2*с)*кгс/см2]";
      x_v_av[i] = sum(x_v[i + zahod * (k - 1), j] for k in 1:numberOfFlueSections / zahod, j in 1:numberOfTubeSections) / (numberOfFlueSections / zahod) / numberOfTubeSections;
      Xi_flow[i] = lambda_tr(Din, ke, Re_flow_av[i]) * Lpipe / Din;
      phi[i] = phi_heatedPipe(wrhop[i], p_flow_v[i] / 100000, x_v_av[i]) "Расчет коэффициента phi";
      if x_v_av[i] < 1 and x_v_av[i] > 0 then
        dp_fric[i] = w_flow_v_av[i] ^ 2 * Xi_flow[i] * bubble_rho_flow_v[i] / 2 / Modelica.Constants.g_n * (1 + x_v_av[i] * phi[i] * (bubble_rho_flow_v[i] / dew_rho_flow_v[i] - 1)) "Потеря давления от трения";
      else
        dp_fric[i] = w_flow_v_av[i] ^ 2 * Xi_flow[i] * rho_flow_v_av[i] / 2 / Modelica.Constants.g_n "Потеря давления от трения";
      end if;
      p_flow_n[i, 1] - p_flow_n[i, 2] = dp_fric[i] + dp_piez "Формула 2-1 из книги Рудомино, Ремжин";
    end for;
    if HRSG_type == MyHRSG.Choices.HRSG_type.horizontalBottom then
    ...
    elseif HRSG_type == MyHRSG.Choices.HRSG_type.horizontalTop then
      H_flow[2] = H_flow[1] + sum(hod[i] * deltaHpipe for i in 1:numberOfFlueSections / zahod) "Расчет высотных отметок для горизонтального КУ";
    elseif HRSG_type == MyHRSG.Choices.HRSG_type.verticalBottom then
      H_flow[2] = H_flow[1] + sum((i - 1) * deltaHpipe for i in 1:numberOfFlueSections / zahod) "Расчет высотных отметок для вертикального КУ";
    else
      H_flow[2] = H_flow[1] - sum((i - 1) * deltaHpipe for i in 1:numberOfFlueSections / zahod) "Расчет высотных отметок для вертикального КУ";
    end if;
...
//Граничные условия для высотной отметки входного коллектора
    ...
    elseif HRSG_type == MyHRSG.Choices.HRSG_type.horizontalTop then
      ...
      H_flow[1] = s2 * (z2 - 1) "Задание высотной отметки входного коллектора";
    end if;
    for i in 2:zahod loop
      D_flow_n[i - 1, 1] = D_flow_n[i, 1];
    end for;
    ...
    for i in 1:zahod loop
      waterOut.p = p_flow_n[i, 2];
    end for;
    waterIn.p = sum(p_flow_n[i, 1] for i in 1:zahod) / zahod;
    for i in 1:zahod loop
      h_flow_n[i, 1] = inStream(waterIn.h_outflow);
    end for;
    waterOut.h_outflow = sum(array(positiveMax(D_flow_n[i, numberOfTubeSections + 1]) * h_flow_n[i, numberOfTubeSections + 1] for i in numberFirstTubeInLastZahod:numberOfFlueSections)) / sum(array(positiveMax(D_flow_n[i, numberOfTubeSections + 1]) for i in numberFirstTubeInLastZahod:numberOfFlueSections));
    waterIn.h_outflow = sum(array(positiveMax(D_flow_n[i, 1]) * h_flow_n[i, 1] for i in 1:zahod)) / sum(array(positiveMax(D_flow_n[i, 1]) for i in 1:zahod));
    annotation(Documentation(info = "<HTML>Модель теплообменника с heatPort. Моделируется несколько ходов. Кипение. Модель воды - Modelica.Media.Water.WaterIF97_ph. Первый заход труб номеруется с 1, второй также с 1. Т.е. во всех заходах поток с одним знаком, и разность давлений с одним знаком (другое описание гибов).</html>"), Diagram(graphics), experiment(StartTime = 0, StopTime = 10, Tolerance = 1e-06, Interval = 0.02), Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {2, 2}), graphics = {Rectangle(lineColor = {0, 0, 255}, fillColor = {230, 230, 230}, fillPattern = FillPattern.Solid, extent = {{-100, 100}, {100, -100}}), Line(points = {{0, -80}, {0, -40}, {40, -20}, {-40, 20}, {0, 40}, {0, 80}}, color = {0, 0, 255}, thickness = 0.5), Text(origin = {-2, 52}, lineColor = {85, 170, 255}, extent = {{-100, -115}, {100, -145}}, textString = "%name")}));
  end onlyFlowHEBoil_6;

  model onlyFlowHEBoil_1
    function positiveMax
      extends Modelica.Icons.Function;
      input Real x;
      output Real y;
...
//p_flow_n[i, j] - p_flow_n[i, j + 1] = dp_fric[i, j] + dp_kin[i, j] + dp_piez[i, j] "Формула 2-1 из книги Рудомино, Ремжин";
        p_flow_n[i, j] - p_flow_n[i, j + 1] = dp_fric[i, j] + dp_piez[i, j] "Формула 2-1 из книги Рудомино, Ремжин";
        D_flow_n[i, j + 1] = homotopy(D_flow_n[i, j] - C1[i, j] - C2[i, j], waterIn.m_flow / zahod) "Уравнение сплошности (формула 3-6 диссертации Рубашкина)";
        C1[i, j] = numberPMCalcSections * deltaVFlow * HTtoMT_sum(drdh_flow[i, :], numberPMCalcSections, j) * HTtoMT(der_h_flow_v[i, :], numberPMCalcSections, j);
//Возможно не верно указан объем (объем для расчета теплообмена а надо для массообмена)
        C2[i, j] = numberPMCalcSections * deltaVFlow * HTtoMT_sum(drdp_flow[i, :], numberPMCalcSections, j) * der(p_flow_v[i, j]);
      end for;
...
//Граничные условия для высотной отметки входного коллектора
  ...
  model onlyFlowHEBoilLite_2
    ...
    Real der_h_n[numberOfFlueSections, numberOfTubeSections + 1] "Производняа энтальпии потока вода/пар";
    Medium_F.Density rho_v[numberOfFlueSections, numberOfTubeSections] "Плотность потока по участкам трубы в конечных объемах";
    Medium_F.Density rho_n[numberOfFlueSections, numberOfTubeSections + 1] "Плотность потока по участкам трубы в узловых точках";
    Modelica.SIunits.DerDensityByEnthalpy drdh_v1[numberOfFlueSections, numberOfTubeSections] "Производная плотности потока по энтальпии на участках ряда труб";
    Modelica.SIunits.DerDensityByEnthalpy drdh_v2[numberOfFlueSections, numberOfTubeSections] "Производная плотности потока по энтальпии на участках ряда труб";
    Modelica.SIunits.DerDensityByEnthalpy drdh_n[numberOfFlueSections, numberOfTubeSections + 1] "Производная плотности потока по энтальпии на участках ряда труб";
    Modelica.SIunits.DerDensityByPressure drdp_v[numberOfFlueSections, numberOfTubeSections] "Производная плотности потока по давлению на участках ряда труб";
  ...
  equation
  //*****Уравнения для потока вода/пар и металла
  //rho_flow_n[1] = Medium_F.density_ph(p_n[1], h_n[1, 1]) "Расчет плотности вода/пар в узловых точках";
  //rho_flow_n[2] = Medium_F.density_ph(p_n[2], waterOut.h_outflow) "Расчет плотности вода/пар в узловых точках";
  //w_flow_n[1] = D_flow_n[1] / rho_flow_n[1] / f_flow "Расчет скорости потока вода/пар в узловых точках";
  //w_flow_n[2] = D_flow_n[2] / rho_flow_n[2] / f_flow "Расчет скорости потока вода/пар в узловых точках";
    w_flow_v = D_flow_v / (sum(rho_v[i, j] for i in 1:numberOfFlueSections, j in 1:numberOfTubeSections) / numberOfFlueSections / numberOfTubeSections) / f_flow;
    for i in 1:numberOfFlueSections loop
      hod[i] = (-1) ^ (i / zahod + (if mod(i, zahod) == 0 then 0 else 1 - mod(i, zahod) / zahod)) "Расчет четный или нечетный текущий ход повехности нагева";
  //Рачет скорости потока в узловых точках
  //Уравнения для расчета процессов теплообмена
      for j in 1:numberOfTubeSections loop
  //Осреднение по конечному объему
        deltaVFlow * rho_v[i, j] * der(h_v[i, j]) = max(0.5 * alfa_flow[i, j] * deltaSFlow * (t_m[i, j] - t_flow[i, j]), 0) - D_flow_v * (h_v[i, j] - h_n[i, j]) "Уравнение баланса тепла теплоносителя (ур-е 3-1d1 диссерации Рубашкина)";
        deltaVFlow * rho_v[i, j] * der_h_n[i, j + 1] = max(0.5 * alfa_flow[i, j] * deltaSFlow * (t_m[i, j] - t_flow[i, j]), 0) - D_flow_v * (h_n[i, j + 1] - h_v[i, j]) "Уравнение баланса тепла теплоносителя (ур-е 3-1d2 диссерации Рубашкина)";
  //Уравнение теплового баланса металла
        deltaMMetal * C_m * der(t_m[i, j]) = Q_flow[i, j] - max(alfa_flow[i, j] * deltaSFlow * (t_m[i, j] - t_flow[i, j]), 0) "Уравнение баланса тепла металла (формула 3-2в диссертации Рубашкина)";
  //Уравнения для heat
        if hod[i] < 0 then
  ...
  //Уравнения состояния
        stateFlow[i, j] = Medium_F.setState_ph(p_v, h_v[i, j]);
        t_flow[i, j] = Medium_F.temperature(stateFlow[i, j]);
        //rho_v[i, j] = Medium_F.density(stateFlow[i, j]);
  //Коэффициент теплоотдачи
        k_flow[i, j] = Medium_F.thermalConductivity(stateFlow[i, j]);
        Pr_flow[i, j] = Medium_F.prandtlNumber(stateFlow[i, j]);
        mu_flow[i, j] = if Medium_F.dynamicViscosity(stateFlow[i, j]) < 1.503e-004 then 1.503e-004 else Medium_F.dynamicViscosity(stateFlow[i, j]);
  //w_flow_v[i, j] = D_flow_v / rho_v[i, j] / f_flow "Расчет скорости потока вода/пар в конечных объемах";
        Re_flow[i, j] = abs(w_flow_v * Din * rho_v[i, j] / mu_flow[i, j]);
        alfa_flow[i, j] = max(0.023 * k_flow[i, j] / Din * Re_flow[i, j] ^ 0.8 * Pr_flow[i, j] ^ 0.4, 1);
  //assert(t_m[i, j] < t_flow[i, j], "Temperatura metalla nije temperaturi potoka (SH)", level = AssertionLevel.warning);
  //Про две фазы
  ...
  //Осреднение по конечному объему
    ...
    D_flow_v = (D_flow_n[1] + D_flow_n[2]) / 2;
  //Основное уравнение гидравлики
  //wrhop = sum(w_flow_v[i, j] for i in 1:numberOfFlueSections, j in 1:numberOfTubeSections) / numberOfFlueSections / numberOfTubeSections * (sum(rho_v[i, j] for i in 1:numberOfFlueSections, j in 1:numberOfTubeSections) / numberOfFlueSections / numberOfTubeSections) * p_v * 10 ^ (-5) "Произведение wrhop для расчета phi [кг/(м2*с)*кгс/см2]";
  //Xi_flow = lambda_tr(Din, ke, sum(Re_flow[i, j] for i in 1:numberOfFlueSections, j in 1:numberOfTubeSections) / numberOfFlueSections / numberOfTubeSections) * Lpipe * z2 / Din;
  //phi = phi_heatedPipe(wrhop, p_v / 100000, sum(x_v[i, j] for i in 1:numberOfFlueSections, j in 1:numberOfTubeSections) / numberOfFlueSections / numberOfTubeSections) "Расчет коэффициента phi";
  //dp_fric = (sum(w_flow_v[i, j] for i in 1:numberOfFlueSections, j in 1:numberOfTubeSections) / numberOfFlueSections / numberOfTubeSections) ^ 2 * Xi_flow * (sum(rho_v[i, j] for i in 1:numberOfFlueSections, j in 1:numberOfTubeSections) / numberOfFlueSections / numberOfTubeSections) / 2 / Modelica.Constants.g_n "Потеря давления от трения";
    lambda_tr = 1 / (1.14 + 2 * log10(Din / ke)) ^ 2;
    Xi_flow = lambda_tr * Lpipe * z2 / Din;
    dp_fric = w_flow_v ^ 2 * Xi_flow * (sum(rho_v[i, j] for i in 1:numberOfFlueSections, j in 1:numberOfTubeSections) / numberOfFlueSections / numberOfTubeSections) / 2 / Modelica.Constants.g_n;
    p_n[1] - p_n[2] = dp_fric;
    D_flow_n[1] - D_flow_n[2] = C1 + C2 "Уравнение сплошности (формула 3-6 диссертации Рубашкина)";
  //C1 = 0;
  //C2 = 0;
    C1 = deltaVFlow * sum(drdh_v1[i, j] * der_h_n[i, j] + drdh_v2[i, j] * der_h_n[i, j + 1] for i in 1:numberOfFlueSections, j in 1:numberOfTubeSections) / numberOfFlueSections / numberOfTubeSections;
    C2 = deltaVFlow * sum(drdp_v[i, j] * der(p_v) for i in 1:numberOfFlueSections, j in 1:numberOfTubeSections) / numberOfFlueSections / numberOfTubeSections;
  ...
  //Граничные условия
    ...
    waterOut.h_outflow = sum(array(max(D_flow_n[2], m_flow_small) * h_n[i, numberOfTubeSections + 1] for i in numberFirstTubeInLastZahod:numberOfFlueSections)) / sum(array(max(D_flow_n[2], m_flow_small) for i in numberFirstTubeInLastZahod:numberOfFlueSections));
    waterIn.h_outflow = sum(array(max(D_flow_n[1], m_flow_small) * h_n[i, 1] for i in 1:zahod)) / sum(array(max(D_flow_n[1], m_flow_small) for i in 1:zahod));
  initial equation
    for i in 1:numberOfFlueSections loop
      for j in 1:numberOfTubeSections loop
        der(h_v[i, j]) = 0;
