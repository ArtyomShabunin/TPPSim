...
equation
  ...
  Dsteam = if noEvent(inStream(fedWater.h_outflow) > h_dew) then D_fw elseif noEvent(inStream(fedWater.h_outflow) < h_bubble) then m_flow_small else max(D_fw * (inStream(fedWater.h_outflow) - h_bubble) / (h_dew - h_bubble), m_flow_small);
//Питательная вода
